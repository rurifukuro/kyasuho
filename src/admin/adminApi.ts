...
export async function fetchOwnTenant(): Promise<KyTenant | null> {
  ...
  // SECURITY DEFINER RPC 経由（0059 列 GRANT で owner_user_id が authenticated 非公開）
  const { data, error } = await supabase.rpc('ky_get_my_tenant').maybeSingle();
...
// ── 通報管理（ky_reports・§15 UGC 4要件）─────────────

export type KyReport = {
  id: string;
  tenant_id: string;
  reporter_user_id: string;
  target_type: 'cast' | 'reservation' | 'tenant';
  target_id: string;
  reason: string;
...
export async function fetchReports(tenantId: string): Promise<KyReport[]> {
  const { data, error } = await supabase
    .from('ky_reports')
    .select('*')
    .eq('tenant_id', tenantId)
