// web/src/shiftTemplates/definitions.ts — シフト表テンプレート定義40種（SPEC §22）
//
// ★このファイルが定義の正準（Web側）。アプリ側 src/shiftTemplates/definitions.ts へは
//   同一内容をコピー同期する（§22「定義40種の正準は web/src/shiftTemplates/definitions.ts」）。
//   変更する時は必ず両方同時に変える。
//
...
export type ShiftPlacement = {
  ...
  /**
   * セル単位の自由配置上書き（ロック解除）。
   * キー = 行インデックス（ヘッダー行を含む）× cols ＋ 列インデックス。
   * 値は画像全体に対する 0-1 比率の矩形。未登録セルはグリッド等分割のまま。
   */
  cellRects?: Record<number, { x: number; y: number; w: number; h: number }>;
};
