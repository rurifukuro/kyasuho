-- 席種に席数(capacity)列を追加＋予約RPCの席数計算を席種合計に変更
-- 受付枠(ky_unlock_windows)のseats列は残すが、予約ロジックでは使わなくなる

-- 1. ky_seat_types に capacity 列追加
ALTER TABLE ky_seat_types ADD COLUMN IF NOT EXISTS capacity INTEGER NOT NULL DEFAULT 1;
ALTER TABLE ky_seat_types ADD CONSTRAINT ky_seat_types_capacity_check CHECK (capacity >= 1);

-- 2. ky_make_reservation RPCを再作成：席数を席種合計から取得
...
begin
  ...
  -- set_minutesとclose_at判定は引き続きunlock_windowsから取得
