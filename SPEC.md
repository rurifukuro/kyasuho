...
40. **出勤・シフト連携の是正パック**（①管理Webの深夜時刻入力24+対応（AdminCasts/AdminSchedule）②指名のサーバー側検証=cast_not_available（RPC v2相乗り可）③手動予約の出勤フィルタ=**要ユーザー判断**＝§35。三面連携自体は2026-07-10監査で健全確認済み）
41. **郵便番号→住所自動入力＋エリア自動選択**（zipcloud API・「住所検索」ボタン方式・対象=管理Web AdminSettings／アプリ StoreProfileModal／キャスト個人情報＝§36。管理Webはエリア欄自動選択必須。DB変更なし。分割=(a)util＋管理Web→(b)アプリ2画面）
...
43. **開発者売上・契約集計ダッシュボード**（ky_revenue_events／ky_platform_admins＋集計RPC3本＋管理Web `#/dev`＝§37。DB/RPC/画面は課金OFFの今でも先行実装可・Webhookへの記帳フック組込みはBILL-1と同時）
44. **キャストシフト提出リマインダー**（ky_shift_requests／ky_shift_submissions／ky_shift_reminder_settings／ky_notification_log＋Scheduled Edge Function `ky-shift-remind`＋手動催促＝§38。§26 #8「シフト希望提出」の実装を包含）
...
### 35-1. 是正① 管理Webの深夜時刻入力（機能差の解消・優先）
...
### 35-2. 是正② 指名のサーバー側検証（§34 RPC v2 に統合）
...
## §36 郵便番号→住所自動入力＋エリア自動選択（2026-07-10 第8次・設計のみ＝実装は別Rev）
...
- **先行実装可（課金OFFの今でも）**: migration（ky_revenue_events／ky_platform_admins／ガード＋RPC3本）＋`#/dev`画面はデータ0件で動く。**リリース初日から記帳が始まる状態にしておくのが本節の狙い**（後から仕込むと初期の売上データが欠落する）
...
- RLS: キャスト本人＝自分の行のINSERT/SELECT（`ky_shifts_self_select` と同型のuser_id→cast解決）。オーナー＝自テナント全行SELECT＋requestsのstatus UPDATE
- 再提出: submitted_at をupsert更新。期限後の提出も可（提出日時は記録に残る）
- **UI**:
  - キャスト側（CastHomeScreen拡張）: 対象期間の希望枠入力（既存シフトUI部品流用・0〜29時＝深夜対応）→「この期間のシフト希望を提出する」ボタン→提出済みバッジ＋提出日時
...
- 実装分割: **(a)** migration（ky_shift_requests／ky_shift_submissions／ky_shift_reminder_settings／ky_notification_log）＋キャスト提出UI＋オーナー承認・提出状況リスト → **(b)** 設定UI＋pg_cron＋`ky-shift-remind`＋手動催促
