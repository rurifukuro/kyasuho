...
- **i18n**：**お客様向け画面（客Web・お客様モード）のみ5言語対応（ja/en/zh/ko/fr）**。提供者アプリ・管理Web・キャストアプリは日本語のみ（2026-07-13ユーザー確定）。全文言`t()`経由
...
- **将来・任意（△）**：Android版（多言語＝客向け5言語は✅確定→実装フェーズで対応）
...
- [x] **フリー/有料の具体ライン**（✅2026-07-13確定＝**無料プランなし**・全モジュール有料・トライアル終了後=履歴閲覧のみ。§52-2/§52-8に反映済み）
- [x] **月額サブスク価格**（✅2026-07-12確定＝§52-2b。8モジュール単品¥5,800〜¥12,800・8個全部=¥39,800。BILL-0ゲート（§52-8）も全項目決定済み）
- [x] **MVPスコープの骨格**（2026-07-05ユーザー指示＝「アプリ＋PC作業サイト＋連携」の三面が必須。§5の★一式で確定。新機能F/H/I込み）
- [x] **i18n範囲**（✅2026-07-13確定＝客向け画面のみ5言語(ja/en/zh/ko/fr)・提供者/管理/キャスト=日本語のみ。§4に反映済み）
- [x] **オーダー機能のフリー/有料境界**（✅2026-07-13確定＝無料プランなし。register モジュール(¥9,800)として提供。トライアル中は全機能利用可・終了後は履歴閲覧のみ）
- [x] **初月無料**（2026-07-06ユーザー決定＝最初の1ヶ月は利用無料。実装は**サーバーサイド・トライアル一本化＝2026-07-10改定承認・Rev70**（ストア標準トライアルは使わない＝§52-3）。月額の実額は引き続き要決定）
...
>
> **【2026-07-12 第15次】サブスク・課金の詳細設計＝§52で確定**（BILLING_DESIGNのSPEC上位化・§34〜§50新機能のモジュール帰属マッピング・実装フェーズBILL-0〜4の§19採番58〜61・ユーザー決定ゲート一覧）。本§の「free/pro 2値」の表は歴史的な叩き台として残す＝**商品モデルの現行確定版はモジュール選択型（§52-2）**。矛盾したら§52が勝つ。
...
51. **お客様モード統合＋モバイルオーダー**（RoleResult拡張＋ky_customer_accounts＋ky_customers.account_id＋ky_customer_follows・卓QR→pending→店承認confirmed・ky_submit_mobile_order RPC・§41ポイント自動化の前提＝§45・2026-07-11第12次。**大型＝サブ分割**(a)DB基盤(b)ロール判定＋ナビ(c)モバイルオーダー(d)ポイント＋会員QR(e)アプリ内予約＋§45-6出勤外指名(f)シフト閲覧。推奨順=a→b→c→d→e→f＝docs/CRITICAL_REVIEW §4参照）
52. ~~SaaS Q&A AI~~（**✅実装完了 Rev130**＝3層アーキテクチャ(FAQ辞書/Sonnet/web_search)＋アプリ HelpChatModal＋管理Web AdminHelp＋reserve_ky_faq_slot＝§46第14次参照）
...
（以下58〜61＝課金実装パック＝§52・2026-07-12第15次。**着手前提=BILL-0ユーザー決定ゲート（§52-8）＋専用プロジェクト分離＋§33 Phase B**。順序はBILL-1→2→3→4固定）

58. **課金台帳基盤（BILL-1）**（migration: ky_billing_subscriptions/ky_billing_customers/ky_payment_events/ky_billing_invoices＋selected_modules等の商品軸列＋`recompute_tenant_entitlements` RPC・`ky-iap-verify`/`ky-apple-notify`・**クーポン2表＋`ky-coupon-redeem`＝チラシ施策を決済より先に単体稼働**・§37のky_revenue_events記帳フック組込み＝§52-1/52-3/52-7）
59. **Stripeカード直販（BILL-2）**（`ky-billing-checkout`/`ky-billing-portal`/`ky-stripe-webhook`・管理Web `#/admin/plan` モジュール選択UI（個数逓減の動的料金プレビュー）・**テストモードE2E＝購入→entitlements反映→解約→降格**。本番鍵切替はユーザー承認ゲート＝§52-4/52-5）
...
66. **複数人会計＋ゲストバック率**（ky_orders.guest_count＋同卓伝票分割＋ky_menu_items.guest_back_rate/guest_back_amount＋伝票ゲストマーク・日報guest_count自動集計連携＝§54-1/54-2。2026-07-13第17次）
67. **Stripe webhook骨組み**（`ky-stripe-webhook` Edge Function skeleton＝checkout.session.completed/invoice.paid/customer.subscription.updated/deleted のルーティング枠＝BILL-2先行。2026-07-13第17次）
...
### 35-3. 是正③ アプリ手動予約の指名ドロップダウン（✅A案確定 2026-07-13）

- **確定＝A案**: 現状維持（オーナー入力は自由＝出勤フィルタなし）。理由＝電話予約の代理入力ではシフト外キャストも指定できて都合が良い。客Webとの挙動差は仕様として明文化。

### 35-5. 出勤予定外キャスト指名希望（§45-6 新機能・2026-07-13ユーザー決定）

シフト表に載っていないキャストでも、常連からの希望があれば出勤するケースに対応。

- **概要**: 客Web予約画面に出勤予定外キャストも「出勤予定外」として表示。お客様は「希望を出す」だけ出せる
- **キャスト側設定**: `ky_casts.accepts_offschedule_nomination`（ON/OFF・本人がアプリ内設定で制御・既定OFF）
- **フロー**:
  1. 客が出勤予定外キャストへの指名希望を送信
  2. キャストアプリへプッシュ通知「予約希望が入りました」
  3. キャストがアプリ上で**承諾** → `ky_shifts` に自動追加＋予約の `nomination_type` を `confirmed` に昇格
  4. キャストが**辞退** → お客様へ辞退通知（客Web予約確認ページに表示。§45実装後はプッシュも追加）
- **DB設計**: `ky_reservations.nomination_type: 'confirmed' | 'offschedule_request'`（NULLは従来の確定指名扱い）
- **既存検証（§35-2 Rev81）との整合**: `cast_not_available` 検証は `nomination_type='confirmed'` のみに適用。`offschedule_request` は検証スキップ（別RPC `ky_request_offschedule_nomination` で処理）
- **承諾後シフト追加**: キャスト承諾→自動でシフト枠追加→予約のnomination_type昇格→通常の予約確定フローに合流
...
### 45-7. トライアル期間終了→機能ロック UX（レビュー A・2026-07-12 批判的検証で追記）

> **対象**: §14 サーバーサイドトライアル（channel='promo'・BILLING_DESIGN §17-1）終了後のダウングレードパス。IAP_ENABLED=false の現 MVP ではトライアル台帳行すら発行しないが、ON化時にここが**初ユーザー体験の最重要分岐点**となる。

- **期限管理**: `ky_billing_subscriptions.current_period_end`（§52-1 で設計済み）。Edge Function `ky-billing-sweep`（§52-4/BILL-3）が期限超過を検知 → `status='past_due'` → n日後 `status='canceled'`
- **ロック範囲**: `recompute_tenant_entitlements` が plan_effective='free' に降格 → features.ts の `isModuleEnabled(module)` が false を返す → **UIレイヤーでブロック**（画面は表示するがアクション不可＋「この機能はプランに含まれません」オーバーレイ）
- **データ保持**: 無料落ちしてもデータ削除しない＝再契約で即復帰（SaaS としての信頼性）。ただしDB容量が膨らむため **90日超過の canceled テナントは通知後アーカイブ**（将来検討・Free プランの DB 500MB 制約に注意）
- **UX**: SettingsScreen に「プラン期限: ○月○日」表示（残7日/3日/1日でバナー強調）＋管理Web ヘッダにも同表示。期限切れ後の初アクセスで「プランが終了しました」モーダル（継続/解約の2択・§52-4 のストア遷移 or Web portal 遷移）

### 45-8. お客様ロールの RLS 攻撃面（レビュー B・2026-07-12 批判的検証で追記）

> **原則**: お客様ロールに対して**既存の owner/cast テーブルへの SELECT/INSERT/UPDATE を一切追加しない**。お客様がアクセスできるのは以下のみ:

| テーブル/RPC | 操作 | ポリシー/認可 |
|---|---|---|
| ky_customer_accounts | ALL（自分のみ） | `user_id = auth.uid()` |
| ky_customer_follows | ALL（自分のみ） | `account_id in (自分のaccount)` |
| ky_point_transactions | SELECT（自分のみ） | `customer_ref in (自分が紐付く ky_customers)` |
| ky_submit_mobile_order RPC | 実行 | token 検証＋authenticated |
| ky_tenants (anon GRANT 列のみ) | SELECT | id/slug/name/genre/business_info/is_suspended（0046で制限済み） |
| ky_menu_items (anon GRANT 列のみ) | SELECT | 0046 列GRANT（SEC-15） |
| ky_casts (公開列のみ) | SELECT | §30 public_profile 列に限定（migration 未作成＝実装Revで追加） |
| ky_shifts (公開列のみ) | SELECT | シフト閲覧用 anon/customer SELECT（migration 未作成＝実装Revで追加） |

- **危険な既存ポリシー**: `ky_orders_owner_rw`（owner_user_id 照合）のみ。customer からは ky_orders/ky_order_items に直接 SELECT/INSERT 不可＝**全てRPC経由**（ky_submit_mobile_order のみ）
- **ky_customers の SELECT をお客様に開けない理由**: internal_notes（店内メモ＝「クレーマー」等）が含まれる。お客様が読めるのは ky_point_transactions 経由の points/memo のみ
- **会員QR（account_id）の推測リスク**: UUID v4＝ブルートフォース不可能。QR は暗号論的ランダム値のため、QR 物理奪取以外の攻撃経路なし。RPC ky_attach_customer_by_account は owner 限定＝お客様自身からは呼べない
- **mobile_order_token の攻撃面**: 244bit（uuid×2結合＝10^73パターン）。伝票クローズで即失効＋pending上限100品。最悪ケース（token漏洩）= 店側に pending が大量に積まれるが、会計金額に乗るのは confirmed のみ＝金銭被害ゼロ。店側UIに「全pending却下」ボタンで対処

---

## §45-6 出勤予定外指名（オフスケジュール指名リクエスト）（2026-07-12 批判的検証で新設）

> **背景**: §35-2 `ky_make_reservation` は「指名キャストが該当時間帯にシフトを持っていなければ `cast_not_available` で拒否」する設計。しかし実務では「推しが出勤予定でなくても指名リクエストを出したい」需要がある（特に§45お客様モード統合後）。SPEC §45-2の「予約」行にもこの想定を組み込む必要がある。

### 45-6-1. 設計方針

- **デフォルトは従来通り**＝出勤予定外の指名は拒否（既存動作を壊さない）
- 店/キャスト単位の**オプトイン方式**: `ky_casts.accepts_offschedule_nomination`（boolean, default false）
- 予約の性質を区別する列: `ky_reservations.nomination_type`（'confirmed' | 'offschedule_request' | null）
  - null = 指名なし予約（フリー来店）
  - 'confirmed' = シフト内キャスト確定指名（従来と同じ＝即座に成立）
  - 'offschedule_request' = 出勤予定外のリクエスト状態（承認待ち）

### 45-6-2. ky_make_reservation v3 の改修ポイント

1. **accepts_nomination チェック**（L89-94）: 従来通り。accepts_nomination=false なら無条件拒否
2. **シフトカバレッジチェック**（L178-189）:
   - キャストがシフトを持つ → `nomination_type = 'confirmed'`（従来通り）
   - キャストがシフトを持たないが `accepts_offschedule_nomination = true` → `nomination_type = 'offschedule_request'`（新）＝予約自体は作成するが「リクエスト中」状態
   - キャストがシフトを持たない AND `accepts_offschedule_nomination = false` → `cast_not_available`（従来通り）

### 45-6-3. リクエスト承認/却下フロー

- **店/キャスト→承認**: offschedule_request の予約を確認 → 「出勤できます」→ ky_shifts にスポット出勤行を INSERT → nomination_type を 'confirmed' に UPDATE
- **店/キャスト→却下**: offschedule_request → 予約ステータスを 'cancelled' に（お客様にプッシュ通知§45-2◯後続。MVP=アプリ内の予約一覧に「指名リクエストが辞退されました」表示）
- **放置防止**: 予約日を過ぎた offschedule_request は自動的に期限切れ扱い（cron不要＝表示時に biz_date < today をフィルタ）

### 45-6-4. UI影響

- **予約台帳**: offschedule_request はバッジ/色分け表示＋承認/却下ボタン
- **キャスト管理**: `accepts_offschedule_nomination` トグル（「シフト外指名リクエストを受け付ける」）
- **客Web/お客様モード**: 指名選択時に「シフト外リクエスト可」のキャストにラベル表示＋送信時に「リクエストのためキャストの確認後に確定します」注記

### 45-6-5. 制約

- 席の自動割当（§3-B）はシフト内枠のみ＝offschedule_request は席をブロックしない（承認時に空き席を割当）
- 予約詳細のキャンセル（§3-A）は offschedule_request でも通常通り可能
- §35-2 の二重予約防止（advisory lock）は offschedule_request にも適用＝同一キャスト同時刻に複数リクエスト不可

---

## §46 SaaS Q&A AIアシスタント（2026-07-12 第14次・**実装済み Rev130**）

> 2026-07-12 批判的検証で§46を実装完了状態に書き換え。初版（第12次）は haiku 単層設計だったが、実装は**3層アーキテクチャ＋Sonnet**で大幅に拡張した。

### 46-0. 要件

- きゃすりんに関するQ&Aに答えるAI。API利用OK。**機能・料金の質問に答え、無関係な質問には答えない**

### 46-1. 構成（3層アーキテクチャ・実装済み）

**Edge Function `ky-faq-ai`**（`supabase/functions/ky-faq-ai/index.ts`）:

| 層 | 判定条件 | モデル/手段 | コスト |
|---|---|---|---|
| **Layer 1: FAQ辞書** | 初回質問（history空）かつ context なし | Bigram Dice 類似度 ≥ 0.62 でマッチ → JSON から回答取得 | ¥0（ローカル文字列比較） |
| **Layer 2: Sonnet** | Layer 1 不発 または 会話2往復目以降 | `claude-sonnet-5` + ナレッジ（cache_control ephemeral）| ~¥2-5/回（キャッシュ効果） |
| **Layer 3: Web検索** | Sonnet が tools `web_search_20250305` を使用（最新情報が必要と判断時）| Sonnet + web_search（max_uses:3）| ~¥5-10/回 |

- **入力**: `{ question: string, history: [{role,content}], context?: string }`
- **ロール判定**: auth.uid() → ky_tenants.owner_user_id → ky_casts.user_id → customer（サーバー側自動判定・クライアントから role を受け取らない＝詐称防止）
- **ナレッジ**: `docs/faq_knowledge.json`（157問＝Q&Aペア配列）。SPECから**利用者向けの機能説明・料金プラン・操作手順**を抽出。SPEC原文は投げない＝内部設計の漏洩防止。**ownerナレッジには「プリンター設定」章を含む**（§44-3 連携受け皿）
- **Layer 検出**: Anthropic API 応答の content blocks に `type: 'server_tool_use'`（tool_name: 'web_search'）が含まれれば Layer 3 / 不含＝ Layer 2。Layer 1 は API 未呼び出し
- **プロンプトキャッシュ**: ナレッジ system block に `cache_control: { type: 'ephemeral' }` → 同一ユーザーの連続質問で入力トークン大幅圧縮

### 46-2. スコープガード＋インジェクション対策（実装済み）

1. **GUARD_PROMPT**（system[0]）: 「きゃすりん専属サポート」役割固定・ナレッジ外は推測せずお問い合わせ案内・税務法務拒否・URL出力禁止・インジェクション明示拒否
2. **ユーザー入力は user ロールのみ**（history も全て user/assistant に変換＝system 混入ゼロ）
3. **サーバー側ガード**:
   - 質問長上限: MAX_QUESTION_CHARS = 1000
   - 履歴上限: MAX_HISTORY_MESSAGES = 6 / MAX_HISTORY_CHARS = 2000
   - **レート制限**: user_id 毎 20回/日 + グローバル 500回/日（`reserve_ky_faq_slot` RPC = increment-then-check 方式・`__global__` sentinel 行）
   - ログ: `ky_faq_logs`（question / answer / layer_used / role / created_at）

### 46-3. UI（実装済み・両面）

| 面 | 実装 | ファイル |
|---|---|---|
| **アプリ** | `HelpChatModal`（FormModalShell＋KeyboardDoneBar）・設定→ヘルプ セクション | `src/components/HelpChatModal.tsx` |
| **管理Web** | `AdminHelp` ページ（/admin/help・ナビ「ヘルプ」）・admin-help-card CSS | `web/src/admin/AdminHelp.tsx` |

- 共通: 履歴はセッション内のみ（サーバーに会話状態を持たない）・Layer1/2/3の使い分けはサーバー透過・回答下部に固定注記「AIによる自動回答です。解決しない場合はお問い合わせへ」＋フォーム導線（行き止まり禁止＝WEB6）
- **文脈付き起動**（§44-3連携）: `context` prop → 初回メッセージに自動添付（プリンター設定ウィザードの「AIに質問」ボタン受け皿。**§44-3プリンターウィザード自体は未実装**＝context配線ポイントのみ用意済み）
- **Enter送信**: `e.key === 'Enter' && !e.shiftKey && !e.nativeEvent.isComposing`（IMEガード）
- 1000文字上限（textarea maxLength）

### 46-4. コスト構造と運営上限

- Layer 1 ヒット率が高いほど API コスト削減（閾値 0.62 は precision 優先＝不確実なマッチで誤回答を避ける）
- グローバル 500回/日 × 平均 ¥5/回 ≒ 最大 ¥2,500/日＝月 ¥75,000 天井。**Free プランでも利用可**（SaaS差別化＝サポートコスト削減効果で回収）
- 将来のコスト圧縮手段: Layer 1 辞書の拡充 / Haiku フォールバック（低複雑度質問自動切替）/ customer ロールへの制限強化
...
- **✅ ユーザー確定（2026-07-13）**: 店舗によって使用する/しないがあるため**設定画面でON/OFF切替可能**にする＝既設計の「タイマー非表示設定（時間無制限の店）」をそのまま実装
...
- **✅ ユーザー確定（2026-07-13）**: 日報の**編集ロック解除にはログインパスワードが必要**（万が一ミスがあった時に編集できるようにする）。パスワードを一度入力すると**そのセッション中は日報画面を自由にいじれる**イメージ＝セッションごとに1回認証。来客数（guest_count）はレジ画面で集計済みの数字を使用（手入力不要）
...
- **✅ ユーザー確定（2026-07-13）**: 給与計算に統合する（設計どおり・既定OFFの設定制で実装）
...
## §52 サブスク・課金 詳細設計＝SPEC確定版（2026-07-12 第15次・設計のみ＝実装は別Rev）

> **位置づけ**: §14（境界の叩き台・歴史的経緯）と `docs/BILLING_DESIGN.md`（Rev68〜71・下位詳細設計639行）を統合し、SPECレベルの確定事項として一本化する。**矛盾したら本§（SPEC）が勝つ**。DB列定義・Webhookペイロード・Edge Function実装詳細は引き続きBILLING_DESIGNを正とする（本§＝アーキテクチャ・商品モデル・機能帰属・UI・フェーズ・ユーザーゲート）。

### 52-1. 確定アーキテクチャ（エンティトルメント単一源泉）

- **販売チャネル4種**（BILLING_DESIGN §1）: A=Apple IAP（15%・Small Business）／B=Google Play（15%）／C=Stripeカード＝管理Web直販（3.6%）／D=銀行振込（Stripeバーチャル口座・1.5%・**半年or年の一括前受けのみ**＝月払い振込はやらない）
- **権利の単一源泉**: 全チャネル → **`ky_billing_subscriptions`（チャネル横断の契約台帳）** → `recompute_tenant_entitlements()`（**service_roleのみ**＝FIN-4拡張・SEC-14） → `ky_tenants.plan`（互換用の粗い値）＋ `entitlements jsonb`（実体）。**クライアントは読むだけ**＝購入直後の楽観アンロック禁止（Webhook/レシート検証の完了を正とする）
- **DB表**: ky_billing_subscriptions（**部分ユニークで1テナント1アクティブ契約**）・ky_billing_customers（プロバイダ顧客IDマップ）・ky_payment_events（**Webhook冪等台帳＝UNIQUE(provider, event_id)**）・ky_billing_invoices（請求書・経理突合）・ky_coupon_campaigns／ky_coupon_redemptions（§52-3）
- **統一状態機械**（全チャネル共通）: `trialing → active → past_due（grace猶予） → canceled → expired`。降格してもテナントデータは消さない（表示ゲートのみ＝解約→再契約で完全復元）
- **Webhook 3原則**: ①冪等（event_id台帳で二重処理拒否）②順序逆転耐性（イベント本文を信じず**最新状態をプロバイダAPIで引き直す**）③処理失敗は5xxを返しプロバイダのリトライに任せる（自前リトライ実装しない）
- **ゲート判定**: `features.ts` の `isModuleEnabled(moduleKey)` **1関数に集約**（GATE-1・骨格実装済みRev69）。`IAP_ENABLED=false` の間は全モジュール有効＝現行挙動不変
...
- **モジュールカタログ確定版＝8本**（✅2026-07-13ユーザー確定。今後機能追加で増える可能性あり）。§34〜§50で増えた全機能の帰属を以下に確定する:

| モジュール | 帰属する機能（§） |
|---|---|
| **base（予約基盤）** | 予約台帳§3-A・受付設定§3-B・キャスト管理§3-C（キャストアカウント含む）・客Web予約§3-D/§34/§36・予約プッシュ通知§3-E・**Q&A AI §46（レート制限つき＝ヘルプを課金ゲートにしない）**・設定/法務§3-G・管理Web基盤§21 |
| shift（シフト表） | §22テンプレ40種/AIデザイン/デイリー/店舗取込§22-5・§31 SNS投稿・§38提出リマインダー・§40強化パック・§33人件費概算 |
| sales（売上・給与） | §23売上/給与・§39メニュー別キャストバック・§49-4スライド時給・キャスト成績ビュー§26#39 |
| register（レジ） | §25オーダー/伝票/会計割引・§29席料自動明細・§49-1卓タイマー・§49-2日報・§49-5指名種別・§44レシート/キッチン印刷・**§47在庫管理** |
| attendance（勤怠） | §23勤怠・§49-3タイムカード打刻 |
| expense（経費） | §27経費/確定申告補助・§42定期固定経費・**§43領収書OCR（Opus・月100枚上限＝✅2026-07-13確定）** |
| analytics（分析） | 期間集計・指名ランキング・比較グラフ・分析ダッシュボード |
| ~~limits（上限解放）~~ | ~~キャスト数無制限・予約数無制限・シフト画像無制限＋透かし除去・独自ブランディング~~ **→廃止（2026-07-13）**: 無料プランなし確定により上限解放の概念が不要化。全有料モジュールは上限なし。独自ブランディングのみ将来モジュール候補 |
| **customer（お客様・新設）** | **§45お客様モード＋モバイルオーダー・§41ポイント/景品・スタンプ§32-2**（客向けAI回答§46もここに準拠） |

- §37開発者ダッシュボードは内部機能＝課金対象外。§44の帳票印刷（expo-print）は各モジュール帰属機能の出力手段＝単体モジュールにしない
- **無料プランなし**（✅2026-07-13ユーザー確定）。全モジュール有料。理由＝2ヶ月のトライアル期間（§52-3）があるため無料枠は不要。**トライアル終了後＝履歴閲覧のみ（新規作成・編集は全てロック）**
- SKU: Web直販は動的（Stripe quantity＋metadata.selected_modules）・**IAP/Playは有限SKU＝簡略9案**（✅2026-07-13確定。BILLING_DESIGN §15-6）
...
| expense（経費/定期固定経費/OCR月100枚） | ¥5,800 | ¥6,800 |
...
- **トライアル終了後に自動課金しない**（明示的な購入操作があって初めて課金＝ダークパターン回避。LP/チラシの安心材料として明記可）。終了時は「全部入り体験→必要モジュールだけ選ぶ」アラカルト営業導線
- 再取得防止: ky_coupon_redemptions を owner_user_id でも記録＋「過去に台帳行（promo含む）を持ったownerのテナントはトライアル不可」をRPC判定。クーポン照合はレート制限（5回/時＝0030 S3パターン流用）
- **クーポン入力は管理Webのみ**。アプリ内には入力欄もQRコードも置かない（§52-4のスマホ新法保守判定と一体）
...
- **アプリ SettingsScreen「プラン」セクション**: `PlanCard`＝現在プラン・モジュール構成・期限・状態バッジ（trialing/active/past_due）。IAP_ENABLED=falseでも「無料プラン（全機能開放中）」表示は先行実装可。ON化時のみ購入ボタン→`ModuleSelector`（モジュール選択→簡略9案SKU解決）→IAP購入→レシート検証→entitlements反映待ち表示
- **管理Web `#/admin/plan`**: モジュール選択UI（チェックボックス＋**個数逓減の動的料金プレビュー**＋期間選択 月/半年/年）→Stripe Checkoutへ→戻りURL→Realtimeでentitlements反映を確認して完了表示。Customer Portalリンク・**クーポン入力欄（三面で唯一ここ）**・銀行振込申込（半年/年のみ）
- **客Web**: 課金UIなし。無料プランは「きゃすりん」バッジ表示のみ（limits契約で非表示化）
- **有料ゲートに当たった時のUX**: 行き止まり禁止（WEB6と同思想）＝「この機能は◯◯モジュールで使えます」＋管理Webプラン画面への導線。**アプリ内ではテキスト誘導のみ**（上記スマホ新法規定に従う）

### 52-6. セキュリティ・監査・運用との接続

- **SEC-14／FIN-4拡張**: `plan`・`entitlements` はservice_roleのみ書込（既存ガード0031）。台帳5表もクライアント直接書込禁止＝全て署名付きWebhook／認証済みRPC経由
- **AUD-1（SEC-15）連動**: 課金関連列（plan/entitlements等の経営列）はanonから列GRANTで隠す＝Rev117の0046パターンを課金列追加migrationでも必ず踏襲
- **鍵管理（R13）**: STRIPE_SECRET_KEY／STRIPE_WEBHOOK_SECRET／Apple共有シークレット等は全てSupabase Edge FunctionのSecret。EXPO_PUBLIC・リポジトリへの埋め込み禁止。**security definer RPCへのservice_role GRANT忘れ**（42501事故＝supabase_edge_function_rpc_grant）に注意
- **W19**: 課金mutationの試打禁止＝検証は**Stripeテストモード／ストアSandboxのみ**。本番鍵への切替はユーザー承認ゲート（BILL-2）
- **§37連携**: Webhook成功処理の末尾で `ky_revenue_events` へ記帳（読み取り専用集計層＝課金本体へ影響なし・BILL-1で組込み＝§19の㊸で先行実装済みの画面が有効化される）
- **§51監査との接続**: AUD-7（招待コード照合のレート制限）は「課金開始前まで」の期限つき＝**BILL-1着手前に消し込む**

### 52-7. 実装フェーズ（BILL-0〜4）と§19採番

| フェーズ | 内容 | §19 |
|---|---|---|
| **BILL-0** | ユーザー決定ゲート（§52-8の全項目）＝**実装着手の前提・番号なし** | — |
| BILL-1 | 課金台帳基盤＝migration（台帳5表＋recompute RPC）＋ky-iap-verify/ky-apple-notify＋**クーポン2表＋ky-coupon-redeem（チラシ施策を決済実装より先に単体稼働できる）**＋§37記帳フック | **58** |
| BILL-2 | Stripeカード直販＝checkout/portal/webhook＋管理Webプラン画面＋テストモードE2E（購入→反映→解約→降格） | **59** |
| BILL-3 | 銀行振込・請求書払い（半年/年のみ）＋sweep＋未入金督促 | **60** |
| BILL-4 | IAP ON化（購入UI＝横断ゲート③）＋アプリ内テキスト誘導＋リンクアウト判断 | **61** |

- **前提順序**: BILL-1は**専用プロジェクト分離後＋§33 Phase Bと同Rev群**（BILLING_DESIGN §13）。IAPを台帳なしの「直接plan更新」で先に作らない（台帳への移行が二度手間＝最初から台帳経由で作る）
- **検証のビルド種別（W21）**: IAP購入UIはExpo Go不可（storeUnavailable Alert案内＝クラッシュさせない）。BILL-4の実機検証はdev build／TestFlightで＝着手前にW21すり合わせ必須

### 52-8. ユーザー決定ゲート一覧（BILL-0・全項目の決定なしに実装Revへ入らない）

| # | 決定事項 | 状態 |
|---|---|---|
| 1 | **モジュール帰属の最終承認**（§52-2表・8本確定・base=有料・customer新設・Q&A AI=base帰属） | ✅確定（2026-07-13） |
| 2 | **価格実額**（モジュール単価・個数逓減テーブル） | ✅確定（2026-07-12・§52-2b） |
| 3 | 無料プラン＝**なし**（全モジュール有料・トライアル終了後は履歴閲覧のみ） | ✅確定（2026-07-13） |
| 4 | IAP SKUラインナップ＝**簡略9案** | ✅確定（2026-07-13） |
| 5 | Apple/Play上乗せ率＝×1.2（心理的価格丸め） | ✅確定（2026-07-12・§52-2b） |
| 6 | インボイス（適格請求書発行事業者）登録の要否 | ⏳ |
| 7 | 特商法（通販）表記ページ＝事業者名・住所・連絡先の表示方法＝**C方式（レンタルオフィス住所）** | ✅確定（2026-07-13・法人化時にレンタルオフィスを構える予定のため） |
| 8 | **Stripeアカウント開設（ユーザー本人操作）** | ✅開設済み（2026-07-13ユーザー申告） |
| 9 | 規約の課金条項追加＋弁護士確認❷（§6）へB2B直販1項目追加 | ✅条項追加済み（2026-07-12・termsOfUse.ts §3新設）・弁護士確認❷は別途 |
| 10 | リンクアウト（10%）採用可否 | BILL-4時点で判断 |
| ✅ | トライアル方式＝サーバーサイド一本化（30日→クーポン60日・自動課金なし） | 承認済み（2026-07-10 Rev70） |

---

## §53 遠隔接客＋お品書きOCR読取り（2026-07-12 第16次・一部実装済み＝Rev122）

### 53-1. 遠隔（リモート接客）の概要

コンカフェ業界では「遠隔」（ビデオ通話でキャストと話す有料接客）が一般的な文化。きゃすりんではこれを2段階で対応する：

- **MVP（本Rev）**: 遠隔の**会計**をレジ・お客様モードに盛り込む。メニュー項目に `remote_price`（遠隔時の価格・NULLなら遠隔非対応）を持たせ、伝票作成時に来店/遠隔を選択して適切な価格を適用できる構造にする
- **将来（MVP外）**: ビデオ通話機能＋遠隔予約フロー＋遠隔会計の自動化（通話終了→伝票自動確定・セット延長の自動加算・遠隔接続状態の管理）

### 53-2. 遠隔価格設計（MVP＝実装済みRev122）

- **DB**: `ky_menu_items.remote_price int DEFAULT NULL`（migration 0049）。NULLの場合は遠隔対応なし（通常価格のみ）
- **アプリUI**: MenuEditModal に「遠隔価格（円・空欄＝対応なし）」入力欄を追加
- **管理Web**: AdminMenu のフォームに遠隔価格入力欄＋テーブルに「遠隔」列を追加
- **会計時の価格選択**: 伝票作成時に「来店/遠隔」を選べるUIを将来追加（→遠隔の場合は`remote_price`を適用）。MVPは手動で遠隔メニューとして伝票に追加する運用も可能

### 53-3. 将来の遠隔ビデオ通話機能（設計メモ・MVP外）

実装時期は未定。以下を将来Revで具体化する：

- **接客形態**: 1対1のビデオ通話（WebRTC or 外部SDK＝Twilio/Agora等）
- **予約との連携**: 客Web予約時に「来店/遠隔」を選択→遠隔の場合は通話ルームを予約時刻に自動生成
- **会計の自動化**: 通話開始→セット料金伝票自動open→延長は時間経過で自動加算→通話終了→会計確定フロー
- **課金チャネル**: 遠隔決済は事前カード決済（Stripe＝§52 BILL-2）が前提（来店後払いの対象外）
- **モジュール帰属**: §52-2のcustomerまたは新規「remote」モジュールとして追加するか要検討

### 53-4. お品書きOCR読取り（MVP＝実装済みRev122）

メニューの初期登録を補助する機能。お品書き画像やPDFを読取り、メニュー項目（品名・価格・遠隔価格・カテゴリ推定）を自動抽出して一括登録できる。

- **制限**: 月20回/テナント（`ky_menu_ocr_usage` テーブルで管理・atomic increment RPCで上限超過防止）
- **Edge Function**: `ky-menu-ocr`（Claude Sonnet 4 vision）。ANTHROPIC_API_KEY=Secret（R13）
- **精度**: とれはんっ！のお品書き読取り（ocr-proxy＝同じAnthropic Vision）と同等精度
- **UI**: 管理WebのAdminMenuに「お品書き読取り」ボタン。ファイル選択→解析→結果テーブル（チェックボックスで選択）→一括追加
- **アプリ側**: src/services/menuOcr.ts にサービス実装済み（UIは管理Webをメインとし、アプリ側UIは後続Revで追加可）
- **遠隔価格**: お品書き画像内に「遠隔」「リモート」と明記された価格がある場合は `remote_price` として抽出

---

## §54 複数人会計・ゲストバック率・LP方針（2026-07-13 第17次・決定のみ＝実装は別Rev）

> 2026-07-13ユーザー指示。本職のコンカフェスタッフからの直接ヒアリングに基づく追加仕様。

### 54-1. 複数人会計（レジ画面の割り勘・注文分割）

- **背景**: レジ画面で同じ卓に複数人のお客さんが同時に座ることがある（グループ来店）。1伝票＝1人の現行設計では対応しきれない
- **要件（ユーザー指示原文）**: 「均等分割で一人当たりの金額を表示したり、同じ卓でも複数人のお客さんを選べるようにし注文を分けたりできるようにする」
- **設計方針**:
  - **A案: 1伝票内の人数指定＋均等分割表示**＝伝票に `guest_count int default 1` を追加。会計時に「合計÷人数＝1人あたり○○円」を表示。実務上最も多いパターン（伝票は1枚のまま・レシートに分割額を印字）
  - **B案: 同卓内で伝票を分ける**＝同じ物理卓（seat/table）に複数のopen伝票を許可。各伝票に別のお客様名を付け、明細を個別に管理。「AさんはドリンクA、BさんはドリンクB」を別精算できる
  - **実装**: A案＋B案の**両方を実装**。UI=伝票作成時に「人数」欄＋「卓内で伝票を分ける」ボタン。日報の guest_count はここの人数合計を使う（§49-2との連携）
- **日報連携**: §49-2の `guest_count` は「当日closed伝票の guest_count の合計」で自動集計（手入力不要になる＝ユーザー確定）

### 54-2. ゲストバック率（インフルエンサー来店時の特別バック）

- **背景**: 特別なインフルエンサーが来店した際にキャストのバック率が通常と異なる場合がある（宣伝効果のある人を接客したキャストへのインセンティブ）
- **要件（ユーザー指示原文）**: 「メニュー欄にゲストバック率を実装し標準時と同じく割合と固定で入力出来るようにする」
- **データ設計**:
  - `ky_menu_items` に列追加: **`guest_back_rate numeric(5,2) null`**（%）＋ **`guest_back_amount int null`**（円）＋ **CHECK（両方非nullは禁止）**＝§39 の back_rate/back_amount と同構造
  - 通常バック（back_rate/back_amount）と排他ではなく**別系統**＝1メニューに「通常バック」と「ゲストバック」の2行を設定可能
- **バック解決ルール（§39-1の拡張）**:
  - 会計時に**伝票を「ゲスト扱い」にマーク**する操作を追加（伝票レーンの「…」メニュー or 会計確定画面に「ゲスト伝票」トグル）
  - ゲスト伝票の場合、back_each の解決を **guest_back_rate/guest_back_amount** → default_back_rate の優先順位で行う（通常のback_rate/back_amountは使わない）
  - **ゲストバックが未設定（null/null）のメニューは通常のバック解決**にフォールバック＝影響は最小限
- **UI**:
  - メニュー編集モーダル: 既存の「キャストバック」欄の下に「ゲストバック（インフルエンサー来店時）」欄を追加＝同じセグメント **［設定なし｜割合%｜固定円］**
  - 伝票レーンのカード: ゲストマーク時に ⭐ バッジ表示（視認性）
  - 管理WebのAdminMenu: テーブルに「ゲストバック」列追加

### 54-3. LP/マーケティング方針＝柔軟な特殊計算対応を売りに

- **背景（ユーザー指示）**: コンカフェ業界の支払い計算は非常に複雑。店ごとに独自のルールがあり、汎用の計算式だけではカバーしきれないケースが確実に出る
- **方針決定**: **契約確約があれば別料金なしで基本2週間以内に特殊計算対応する**ことをLPのセールスポイントにする
  - 「料金変わらず別途対応可能」＝追加開発費不要
  - 「基本2週間以内」＝対応スピードの明示（ただし詳細な対応日数は相談時に確定＝LP上は「基本2週間以内（時期は相談時に確定）」の表記）
- **LPへの反映（§52-6 or LP専用ページ）**: 大項目として「柔軟さと迅速さ」を前面に：
  - ✅ 料金変わらず＝追加費用なしで特殊計算ロジックに対応
  - ✅ 基本2週間以内＝相談から2週間以内で実装（時期は要相談）
  - ✅ 現場の声を即反映＝契約店の要望を直接聞いて機能改善
- **位置づけ**: きゃすりんの最大の差別化＝大手SaaSにはできない**個別対応の速度と柔軟性**。価格で競合するのではなく「うちの店のルールにそのまま対応してくれる」安心感で選ばれる
