...
- ★ **ポイント・景品（クーポン）設定**（「何円の支払いで何ポイント」「何ポイントで何の景品」を店側で設定＝§41・2026-07-10第11次。会計連動の自動付与は姉妹アプリ customer_ref 導入と同時＝§32-2）
...
36. **スタンプ・クーポン**（#42のスタンプ部分＝2026-07-06ユーザー決定で計画入り。姉妹アプリと同時期に実装＝§32-2。会計連動自動加算・クーポン確認ポップアップ。**ポイント制の設定・景品カタログは§41で先行仕様化＝2026-07-10第11次・スタンプはポイント制に包含**）
...
47. **ポイント・景品設定**（ky_point_settings／ky_point_rewards＋設定UI（アプリ/管理Web）。会計連動の自動付与・使用は customer_ref 導入（㊱姉妹アプリ）と同時に有効化＝§41・2026-07-10第11次）
...
- 段階: **設定・景品カタログ（41-1前半＋41-2）は姉妹アプリ前でも実装できる**（店が制度を先に決めて店内掲示・紙運用から始められる）。付与・使用の**自動化はお客様識別子 `customer_ref`（§32-2）の導入と同時に有効化**
...
| `ky_point_transactions` | id, tenant_id, **customer_ref**, order_id null, kind('earn'/'redeem'/'adjust'), points（redeemは負）, reward_id null, created_at | ポイント台帳（**append-only・残高=Σ**＝FIN思想。訂正はadjust行）。**customer_ref の実体（姉妹アプリアカウント）が無い間は稼働しない**＝§32-2と同時に有効化 |
...
- 客Web・姉妹アプリへの制度公開（「当店は500円=1pt」表示）は姉妹アプリ設計時に決定（◯枠）
...
- 実装分割: **(a)** migration（settings/rewards）＋設定UI（アプリ/管理Web）＝先行実装可 → **(b)** transactions＋closeOrderフック＋レジポップアップ＝㊱（姉妹アプリ・customer_ref）と同時
