...
- 作成日: 2026-07-03 ／ 最終改訂: 2026-07-07 第7次（**シフト表取込の改善仕様§22-5新設＝取り込み3モード化（記入済み／空テンプレ決定論グリッド検出／任意画像背景）＋ShiftPlacementV2境界配列＋セル個別微調整。仕様のみ・実装は後Rev＝§19の㊳**）。前回: 同日第6次（**金融・セキュリティ強化設計§33新設＝FIN-1〜8／SEC-11〜13・Phase A〜D・設計のみ実装は別Rev**）。2026-07-06 第5次（**会計時の割引・クーポン§25-7＝店舗独自キャンペーン対応＋姉妹アプリのキャスト個人ランキングを不実装へ§32**。同日第4次＝初月無料トライアル§14＋姉妹アプリ（お客様向け）構想とスタンプ/クーポン・ランキング集計の仕込み§32＋第3次棚卸しの◯/△を実装計画へ組込み§19の㉚〜㊲。同日第3次＝シフト表拡張＝デイリー出勤表§22-2・店舗テンプレ取込§22-3・プレビュー固定§22-4＋SaaS機能ネット調査棚卸し§26-3。同日第2次＝ユーザー10項目指示の反映＝レジお客様名/一時保存（§25-3）・経費/確定申告補助（§27）・入力UX/表示/用語是正（§28）・席種/席料（§29）・キャスト写真2種（§30）・シフト表SNS投稿（§31）・棚卸し§26-2追加。同日第1次＝オーダー管理§3-K/§25・§26新設）
...
- ◯ **店舗テンプレ取込の改善＝取り込み3モード化＋セル個別微調整**（空テンプレの決定論グリッド検出・任意画像を背景に・不均等グリッド対応＝§22-5・2026-07-07ユーザー指示で仕様化済・実装は後フェーズ）
- ◯ 週間/2週間表示バリエーション
- ◯ テンプレートのお気に入り保存
...
### 初月無料トライアル（2026-07-06ユーザー決定＝確定事項）

- **プロの最初の1ヶ月は利用無料**。自前実装はせず**ストア標準のお試しオファー**を使う：
  - iOS＝ASCのサブスクリプション **Introductory Offer（Free Trial・1ヶ月）** を `kyasuho_pro_monthly`/`kyasuho_pro_yearly` の両方に設定
  - Android＝Play Console のサブスクリプション無料試用（free trial）設定
- **コード側の追加分岐は不要**：トライアル中もレシート検証→`ky_tenants.plan='pro'` が立つ（既存エンティトルメント経路のまま）。期限切れ＋未課金→plan戻し。同一Apple ID/Googleアカウントの再取得防止はストア側が管理＝自前で trial_started_at を持つ方式より不正・端末替えに強い
- 訴求面：無料プランが既にあるので、トライアルは**アップグレード導線**として見せる（SettingsScreen の PlanCard に「1ヶ月無料で試す」＝表記は localizedPrice 同様ストア取得値ベース・固定文字で価格や期間をハードコードしない＝R29と同思想）
- Web側にStripe等の独自決済を将来置く場合も同条件（初月無料）を揃える（プラットフォームで差をつけない＝下記「三面共有」の方針どおり）
...
38. **シフト表取込3モード化＋セル個別微調整**（記入済み/空テンプレ/任意画像背景のモード選択UI・空テンプレの決定論グリッド検出・ShiftPlacementV2境界配列＋境界線ドラッグ/セル個別オフセット・モードC可読性ガード＝§22-5。2026-07-07ユーザー指示で仕様化済・**実装着手は指示待ち**。実装順=C→V2+境界ドラッグ→B）
...
- **改善仕様（取り込み3モード化＝空テンプレ決定論解析・任意画像背景・セル個別微調整）＝§22-5**（2026-07-07ユーザー指示。本節Rev61実装の後継設計）
...
### 22-5. 店舗テンプレート取込の改善＝取り込み3モード化＋セル個別微調整（2026-07-07ユーザー指示・本節は仕様のみ＝実装は後Rev・§19の㊳）
...
#### 取り込み3モード（アップロード直後にモード選択UIを置く）

現行の「AIで解析」単独ボタンを、アップロード直後の3択に置き換える：

- **モードA「記入済みシフト表」**（現行維持）：`ky-shift-analyze`（Claude Vision）で解析→初期配置。既存フローそのまま（内部表現のみ後述V2へ移行・挙動不変）。
- **モードB「空のテンプレート」**（新設）：文字/データ未記入のテンプレート画像。**決定論グリッド検出を正**とする2段解析（後述）。
- **モードC「好きな画像を背景に」**（新設）：解析なし。任意の写真/イラスト（店内装・キャライラスト・季節素材等）を背景に敷き、既定の安全配置でタイトル＋グリッドを重ねる→手動調整。**グリッドを持たない画像でも確実に使える保険モード**＝A/Bの解析失敗時のフォールバック先も「モードCに切り替えて手動配置」に一本化（現行の「解析失敗＝行き止まり」を解消・WEB6思想）。

#### モードB：空テンプレートの決定論グリッド検出

とれはんっ！画像解析（map-analysisスキル／mapkit.py）の核心思想＝**「構造検出は決定論アルゴリズム・AIはシードと照合のみ」**をシフト表に移植する。

1. **決定論検出（正・クライアント側Canvas）**：グレースケール化→適応二値化→**水平/垂直の投影プロファイル**（行方向・列方向の暗ピクセル密度ヒストグラム）→ピーク検出で罫線位置を抽出→`colBounds[]`/`rowBounds[]`（セル境界の相対座標配列）を得る。Canvas 2D APIのみ＝**依存パッケージ追加なし・Edge Function往復なし・APIコスト0円・レート制限消費なし**。
2. **Vision補完（従）**：罫線ピークが検出閾値未満（罫線レス・装飾に埋もれる等）の場合のみ `ky-shift-analyze` へフォールバック。リクエストに `mode: 'empty' | 'filled'` ヒントを追加し、空テンプレ用プロンプトへ分岐（「文字記載なし・罫線/枠の構造だけを推定せよ」）。**SSRF対策・レート制限・8MB上限・image/*検査は現行ガードのまま変更しない**（Rev63で監査済みの防御を崩さない）。
3. ヘッダ行（曜日行）の有無は自動判定しない＝UIトグル `hasHeaderRow` で人が指定（現行と同じ・誤判定より確実）。
...
- `cellBgAlpha` スライダー＝セル下敷きの半透明白/黒（写真の上でも文字が読める）。
- `textOutline` トグル＝文字縁取り。
- 初期配置は安全既定値（titleArea＝上部中央・グリッド＝下寄せ幅80%）→手動調整へ。
...
#### 実装順（コスト小→大・着手はユーザー指示待ち）

**C（解析なし＝レンダラー拡張とUIのみ）→ V2データモデル＋境界線ドラッグ → B決定論検出**。モードAはV2内部移行のみで挙動不変＝最初のRevに同梱。
