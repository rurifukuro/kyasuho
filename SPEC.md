...
※§48（競合ベンチマーク提案＝卓タイマー・日報ほか）の採用分は**ユーザー決定後に採番**する⏳
...
- 設定画面「プリンター」＝ ①同一Wi-Fi接続の案内 → ②**IPアドレス入力**（本体テスト印字に記載・ヘルプに機種別の調べ方）＋ePOS既知ポートへの疎通プローブ → ③**テスト印刷ボタン**（1タップで確認） → ④保存
- 保存先: `ky_tenants.printer_config`（jsonb・`{type:'epos', ip, port, paperWidth}`）＝管理Web/アプリ共有。未設定時はレシートボタン非表示（行き止まりUIにしない＝WEB6同思想）
- 印字失敗時: エラー種別表示（未接続/用紙切れ）＋「画面提示に切替」フォールバック（現行運用＝§26-16を残す）
...
- **ナレッジ=システムプロンプト埋め込み方式**（RAG不要の規模）: `docs/faq_knowledge_owner.md`／`faq_knowledge_customer.md` の2本を新設し、SPECから**利用者向けの機能説明・料金プラン・よくある操作手順**を抽出して整備（SPEC原文は投げない＝内部設計・セキュリティ設計の漏洩防止）。**ロール別にナレッジを出し分け**（お客様には管理機能・料金の内部条件を出さない）。料金は§14/BILLING_DESIGN確定値のみ記載・未確定項目は「準備中」と書く（ハードコード価格の陳腐化はナレッジ1ファイル更新で吸収）
...
## §48 競合ベンチマーク＝Dシステム/YONAREZI機能調査と採用提案（2026-07-11 第12次・**提案＝採用可否はユーザー決定⏳**）
...
| **卓タイマー・延長アラート** | ◯のまま未実装（§25-3） | **★格上げ推奨**＝時間制セットのコンカフェで「延長忘れ=取りこぼし」を直接防ぐ。伝票にセット開始時刻は既にある＝実装軽い |
| **日報（営業日クローズ）** | 無し（日別集計はあるが「締め」概念なし） | **★推奨**＝1日の終わりに売上/組数/キャスト別/現金過不足を1枚に自動生成→オーナーへプッシュ。ky_sales/ky_ordersから生成でき実装軽い・両競合の共通機能 |
| **タイムカード打刻** | 勤怠記録は店側手入力（§3-H・§26-9） | **◯提案**＝キャスト側アプリに出退勤打刻→実働時間が給与計算（§23）の時給部へ自動連動。基盤（ky_attendance・キャストログイン）は実装済み |
| スライド時給（成績連動の時給テーブル） | 無し（時給は固定） | **◯提案**＝ky_payroll_settings に時給テーブル（月間売上/指名数の閾値→時給）。YONAREZIの目玉だがコンカフェでの普及度は店による＝需要を見て |
| 本指名/場内指名の区別 | 指名は1種類 | **◯提案**＝キャバ由来の文化。コンカフェは「推し」単一で回る店が多く、区別が必要な店向けの設定制（既定OFF） |
...
- **次アクション**: ★2件（卓タイマー・日報）と◯3件の採用可否は**ユーザー決定⏳**→採用分を§19へ採番
