...
- [ ] **フリー/有料の具体ライン**（§14に設計案→**現行確定版はモジュール選択型＝§52-2の帰属表・最終承認はBILL-0ゲート＝§52-8**。無料コアの上限数値もそこで確定）
- [ ] **月額サブスク価格**（§14に推奨レンジ。店舗向けSaaS＝個人向けより高め。競合コンカフェGoスタンダード¥4,800/月が参考。**実額の決定項目一覧＝§52-8（モジュール単価・個数逓減・半年/年割引率・ストア上乗せ率）**＝価格判断ゲート）
...
| モジュール | 帰属する機能（§） |
|---|---|
| **（無料コア）** | 予約台帳§3-A・受付設定§3-B・キャスト管理§3-C（キャストアカウント含む）・客Web予約§3-D/§34/§36・予約プッシュ通知§3-E・**Q&A AI §46（レート制限つき＝ヘルプを課金ゲートにしない）**・設定/法務§3-G・管理Web基盤§21 |
...
| expense（経費） | §27経費/確定申告補助・§42定期固定経費・**§43領収書OCR（AI原価あり→月次回数上限つき・上限値はBILL-0）** |
| analytics（分析） | 期間集計・指名ランキング・比較グラフ・分析ダッシュボード |
| limits（上限解放） | キャスト数無制限・予約数無制限・シフト画像無制限＋透かし除去・独自ブランディング（客Webバッジ非表示） |
| **customer（お客様・新設）** | **§45お客様モード＋モバイルオーダー・§41ポイント/景品・スタンプ§32-2**（客向けAI回答§46もここに準拠） |

- **customer は BILLING_DESIGN §15カタログ（8本）への9本目の追加**＝本§がカタログを上書きする（BILLING_DESIGN側は次回更新時に追従）
