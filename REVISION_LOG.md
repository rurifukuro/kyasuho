...
## Rev91（2026-07-11）§42〜§48新設＝固定経費・領収書OCR/PDF・印刷連携・お客様モード統合・Q&A AI・在庫管理・競合提案（設計のみ・第12次8項目パック）

ユーザー第12次指示（8項目）の詳細設計をSPEC.mdへ加筆。**コード変更なし＝設計のみ**。Web調査2件実施（楽楽精算の証憑出力仕様・競合Dシステム/YONAREZI機能）。

### 新設セクション（「横断ゲート」直前に挿入）
- **§42 定期固定経費**: ky_recurring_expenses（テンプレ）＋月ごと実体化＝金額スナップショットで**過去月不変**（ユーザー要件の核心）。UNIQUE INDEX(source_recurring_id×月)で冪等・ky_recurring_expense_skipsで「この月だけ削除」・未生成月まとめ補完
- **§43 領収書OCR・PDF・月次証憑レポート**: 現状回答（写真添付=receipts.ts実装済み・**OCRは未実装**）→Edge Function `ky-receipt-ocr`（claude vision・日付/金額/支払先プリフィル→人が確認）・明細単位PDF・**月次経費レポートPDF（表紙→明細表→証憑ページ）＋経費CSV**＝楽楽精算の「申請書＋証憑＋仕訳CSV」構成を個人店向けに再構成。「電帳法対応」は謳わない
- **§44 印刷・プリンター連携**: 帳票=expo-print（AirPrint/Mopria・ゼロ設定）／レシート・キッチン伝票=**EPSON ePOS-Print API**（LAN内HTTP POST・ネイティブモジュール不要=Expo Go互換）・設定ウィザード（IP＋疎通プローブ＋テスト印刷→ky_tenants.printer_config）
- **§45 お客様モード統合**: **姉妹アプリ構想（§32）を廃止し本体アプリへ統合**（方針転換）。RoleResultに customer 追加・ky_customer_accounts＋ky_customers.account_id（customer_ref=ky_customers.idで確定）・ky_customer_follows・**モバイルオーダー=卓QR→pending→店承認confirmed**（ky_submit_mobile_order RPC・決済しない=FIN-8）・開発用ロール切替=SettingsScreen最下部（__DEV__&&features二重ガード・RLS権限は実ユーザーのまま）
- **§46 SaaS Q&A AI**: Edge Function `ky-faq-ai`（claude-haiku）・ロール別ナレッジMD（SPEC原文は投げない）・スコープガード3層＋レート制限20回/日＋ky_faq_logs・料金質問対応（未確定は「準備中」）
- **§47 在庫管理**: ky_inventory_items＋ky_inventory_moves（append-only台帳・残高=Σ）・closeOrder自動減算・**マイナス在庫許容＋警告（会計を止めない）**・閾値アラート・棚卸adjust・AdminInventory
- **§48 競合ベンチマーク**: Dシステム/YONAREZI調査→対応表（在庫✅採用=§47・卓タイマー★推奨・日報★推奨・打刻/スライド時給/指名種別◯・売掛/スカウトバック✗）。**採用可否はユーザー決定待ち⏳**

### ポインタ編集
- §3-F（経費に§42/§43）・§3-K（在庫§47/印刷§44/モバイルオーダー★転換）・**§3-L「お客様モード」新設**・§3-G（Q&A AI§46/プリンター設定§44-3）
- §19: ㊸〜㊽（48〜53）追加＝§42〜§47の実装計画（§48採用分はユーザー決定後採番）・㊱㊼の姉妹アプリ表現を§45へ更新
- §25-6△リスト・§25-7・§26 #15/#16/#17（★格上げ注記）・§27（§42/§43参照）
- §32見出し直下に方針転換注記（仕込み3点は§45が継承・ランキング公開層32-3の設計は不変）・§41の姉妹アプリ表現4箇所を§45参照へ

### Rev採番メモ
- 前セッションではRev85予定だったが、並行実装セッションがRev85〜90を採番済みだったため**Rev91**で採番（ドリフト是正ゲート＝REVISION_LOG最大Rev基準）

### 検証
- コード変更なし（SPEC.md＋REVISION_LOG.mdのみ）＝tsc対象外

---

## Rev92（2026-07-11）§49〜§50新設＝競合採用5機能の詳細設計・DDD先行手当て・プリンター設定説明UI詳細化（設計のみ・第13次3項目）

ユーザー第13次指示の設計加筆。**コード変更なし＝設計のみ**。

### 1. 競合提案の採用（★推奨2件＋◯検討3件＝全採用・§48判定表を✅採用へ更新）→ §49新設
- **49-1 卓タイマー（★）**: ky_orders.set_deadline_at スナップショット・延長=セット明細追加とRPCで締切同時更新（二重管理しない）・残時間バッジ緑→黄→赤・残5分ローカル通知・金額には一切影響しない（FIN-3不変）
- **49-2 日報=営業日クローズ（★）**: ky_daily_reports（business_date UNIQUE・cash_expected/actual/diff）・open残警告→自動集計→実査額入力→確定・締め後は当日データロック（締め直しは監査ログ）・§42/§43/§23の数字の信頼性の土台
- **49-3 タイムカード打刻（◯）**: CastHomeScreenに出退勤ボタン→既存ky_attendance列へ書込RPC・店側修正はedited_by_ownerフラグ・給与実働プリフィル連動
- **49-4 スライド時給（◯・既定OFF）**: ky_hourly_rate_tiers閾値テーブル・月次給与生成時に自動選択（提案・確定は人）・確定済み給与に遡及しない（FIN-2）・プレビュー付き設定UI
- **49-5 本指名/場内指名（◯・既定OFF）**: 第1弾=メニュー分割方式（§39メニュー別バック活用でDB変更ゼロ）・第2弾=nomination_kind列（必要になったら）

### 2. §44-3 プリンター設定ウィザードの説明UI詳細化
- ステップカード形式（STEP 0対応機種説明〜STEP 4テスト印刷）・**IPアドレスの取得方法を平易文＋3択アコーディオンで説明**（A=本体の紙送りボタン長押しでステータスシート印刷（推奨）／B=Epson TM Utilityアプリ／C=ルーター管理画面）・「IPアドレス＝Wi-Fi内のプリンターの住所」定義文・失敗時は原因別の平易な対処文
- **各ステップに「うまくいかない時はAIに質問」＝§46 Q&A AIを文脈付き起動**（ステップ番号＋直前エラーを自動添付）。§46-1にownerナレッジ「プリンター設定」章必須を追記・§46-3に汎用contextプロップ設計を追記
- ◯後続=「かんたん探索」（/24並列疎通プローブで候補自動提示）

### 3. §50 DDD先行手当て（詳細設計・フルDDDはやらない）
- **50-1 コンテキストマップ**: 10コンテキストの依存方向図＝依存は矢印方向のみ・跨ぐ書込はcloseOrder等の集約点だけ
- **50-2 ユビキタス言語用語集**: 11用語の表（伝票/明細/会計/営業日/セット/指名/バック/ポイント/顧客vs.お客様アカウント/実体化/締め）＋「新用語は表に追記してから命名」運用
- **50-3 純関数分離規約D-1〜D-5**: src/domain/新設（supabase/fetch/React import禁止）・新規コードから適用（一括移動禁止）・切り出し対象8関数表（§42〜§49の実装Revで最初からdomainに置く）・アプリ側が正→Webへコピー同期コメント必須・RPCは残す（数式変更はSQL+domain+表の3点同時更新）・境界値入出力例をREVISION_LOGに最低3件
- **50-4**: やらないこと（Entity/Repository化等）・再判定=Supabase専用プロジェクト分離時

### ポインタ編集
- §3-C（指名種別◯）・§3-F（日報★・スライド時給★）・§3-H（打刻★）・§3-K（卓タイマー◯→★）・§3-G/§46（AI文脈起動・プリンターFAQ章）
- §19: 54（卓タイマー＋日報）・55（打刻）・56（スライド時給＋指名種別）・57（DDD先行手当て）採番（第12次の「§48採用分は決定後採番」注記を置換）
- §25-3・§25-6・§26 #6/#9（★格上げ注記）・§48（見出し＋判定表＋次アクション→採用決定済みへ）
...
## Rev93（2026-07-11）§41(a) ポイント・景品設定（設定UI＋景品カタログCRUD）

SPEC §41 の前半（a）を実装。ポイント制度ON/OFF＋円/pt単価の設定と、景品カタログCRUD。
§41(b)のポイント付与・使用台帳（ky_point_transactions）はcustomer_ref（§32-2）導入後に実装。

### Migration 0036 (`supabase/migrations/0036_ky_point_settings_rewards.sql`)
- **ky_point_settings**: tenant_id PK, enabled, yen_per_point (CHECK>0, default 500), updated_at。RLSはowner限定
- **ky_point_rewards**: id UUID PK, tenant_id FK, points_required (CHECK>0), name, description, is_active, sort_order, created_at, updated_at。RLSはowner限定。idx_ky_point_rewards_tenant

### 型定義
- `src/types/index.ts`: PointSettings, PointReward 追加
- `web/src/lib/types.ts`: KyPointSettings, KyPointReward 追加

### アプリ側サービス (`src/services/points.ts`)
- fetchPointSettings / savePointSettings（upsert）/ DEFAULT_POINT_SETTINGS
- fetchPointRewards / createPointReward / updatePointReward / deletePointReward

### Web管理画面
- **adminApi.ts**: fetchPointSettings / upsertPointSettings / fetchPointRewards / upsertPointReward / deletePointReward
- **AdminSettings.tsx**: PointSettingsSection 追加（ON/OFF + 単価入力 + 換算プレビュー + 景品カタログテーブル + 追加/編集/削除フォーム）

### アプリ側UI
- **PointSettingsModal**: FormModalShell + ON/OFF Switch + ステッパー式単価 + 景品一覧表示（追加・編集はPC管理画面へ誘導）
- **SettingsScreen.tsx**: 「ポイント」セクション追加（顧客管理と連携の間）

### i18n (`src/i18n/strings.json`)
- settings.sectionPoint, points.title, points.settingsSub, points.enable, points.enableHint, points.yenPerPoint, points.yenExample, points.yenInvalid, points.rewardCatalog, points.noRewards, points.rewardHint, points.active, points.inactive
...
## Rev95（2026-07-11）§49-3 タイムカード打刻（キャスト自己打刻＋edited_by_ownerフラグ）

SPEC §49-3 実装。キャストがCastHomeScreenから出勤/退勤を打刻。基盤（ky_attendance + check_in_at/check_out_at列）は§26-9で実装済み＝RPCと UIの追加が主な作業。

### Migration 0038 (`supabase/migrations/0038_ky_timecard_punch.sql`)
- **ky_attendance.edited_by_owner** (boolean default false): キャスト申告値と店修正値の区別＝給与トラブル防止の最小記録
- **ky_attendance_cast_select** RLS: キャストが自分の勤怠を閲覧可能に（`cast_id in (select id from ky_casts where user_id = auth.uid())`）
- **ky_cast_punch(p_direction text) RPC** (SECURITY DEFINER):
  - 'in': 出勤打刻。既存行なければ status='present' で INSERT、既存行あれば check_in_at を UPDATE（二重打刻ガード: already_punched_in で RAISE）
  - 'out': 退勤打刻。未出勤なら not_punched_in / 既退勤なら already_punched_out で RAISE
  - 日本時間（Asia/Tokyo）で日付・時刻を決定
  - 結果を jsonb で返却（date, check_in_at, check_out_at, status）

### 型定義
- `src/types/index.ts`: Attendance に editedByOwner: boolean 追加
- `web/src/lib/types.ts`: KyAttendance に edited_by_owner: boolean 追加

### アプリ側
- **services/timecard.ts** (NEW): castPunch(direction) = ky_cast_punch RPC 呼出 / fetchTodayAttendance(castId) = 今日の打刻状態取得
- **services/attendance.ts**: AttendanceRow/rowToAttendance に edited_by_owner 追加
- **screens/CastHomeScreen.tsx**:
  - TimecardSection コンポーネント追加（シフト提出セクションの上に配置）
  - 状態表示: 未出勤（clock-outline・グレー）/ 出勤中（clock-check・緑・時刻表示）/ 退勤済（clock-remove・グレー・時刻範囲表示）
  - 出勤ボタン（緑・login アイコン）/ 退勤ボタン（赤・logout アイコン）
  - 確認ダイアログ→RPC呼出→即時UI更新

### Web管理画面
- **adminApi.ts**: upsertAttendance に editedByOwner パラメータ追加（店側修正時は true を設定）
- **AdminAttendance.tsx**:
  - テーブルに「入力元」列追加（本人打刻=緑バッジ / 店舗修正=黄バッジ）
  - CSV出力に入力元列追加
  - 店側での勤怠記録/修正時に edited_by_owner=true を自動設定

### i18n
- 13キー追加: timecard.sectionTitle / notPunchedIn / working / done / punchIn / punchOut / confirmIn / confirmOut / confirmReIn / punchInDone / punchOutDone / punchError

### 検証
- アプリ `npx tsc --noEmit` EXIT:0
- Web `npx tsc -b` EXIT:0
- 境界値: 二重出勤打刻→already_punched_in例外 / 未出勤で退勤→not_punched_in例外 / 二重退勤→already_punched_out例外
- **Migration 0038は本番未適用**（承認ゲート）

---

## Rev96（2026-07-11）§49-4 スライド時給（実績連動の段階時給テーブル）

### DB（Migration 0039）
- `ky_payroll_settings` に `slide_enabled boolean not null default false` 追加
- `ky_hourly_rate_tiers` テーブル新設（id/tenant_id/metric/threshold/hourly_rate/sort_order）
  - metric は `monthly_sales` または `monthly_nominations`（CHECK制約）
  - threshold >= 0、hourly_rate > 0（CHECK制約）
  - RLS: owner-only ポリシー

### ドメイン層（§50-3 純関数）
- `src/domain/payroll/slideTier.ts`: `resolveTierRate(tiers, metricValue, fallbackRate)` — しきい値降順ソート→最初に超えたtierの時給を返す。未達なら基本時給

### アプリ側
- **types/index.ts**: PayrollSettings に `slideEnabled` 追加、`SlideMetric` 型・`HourlyRateTier` 型新設
- **services/hourlyRateTiers.ts**: CRUD（fetch/create/update/delete）
- **services/payroll.ts**: PayrollSettingsRow/rowToSettings/DEFAULT/Input/save にslideEnabled反映
- **PayrollView.tsx**: EffectiveSettings に slideEnabled 追加、設定保存時に既存値を維持

### Web管理画面
- **lib/types.ts**: KyPayrollSettings に slide_enabled、KySlideMetric 型・KyHourlyRateTier インターフェース新設
- **adminApi.ts**: savePayrollSettings に slideEnabled 追加、fetchHourlyRateTiers/upsertHourlyRateTier/deleteHourlyRateTier 新設
- **AdminPayroll.tsx**:
  - 給与設定フォームに「スライド時給を有効にする」チェックボックス追加
  - 有効時にティア一覧テーブル表示（指標/しきい値/時給/編集/削除）
  - ティア追加・編集フォーム（指標select/しきい値/適用時給）
  - ティア削除は確認ダイアログ付き

### 検証
- アプリ `npx tsc --noEmit` EXIT:0
- Web `npx tsc -b` EXIT:0
- **Migration 0039は本番未適用**（承認ゲート）

---

## Rev97（2026-07-11）§47 在庫管理（品目マスタ＋append-only入出庫台帳＋管理Web UI）

### DB（Migration 0040）
- `ky_inventory_items` テーブル新設（品目マスタ: name/unit/menu_item_id/stock_qty/alert_threshold/is_active/sort_order）
- `ky_inventory_moves` テーブル新設（append-only入出庫台帳: item_id/kind(in/sale/adjust/out)/qty/order_id/memo）
  - 残高の真実はΣ(moves.qty)＝FIN思想と同型
  - stock_qty はキャッシュ（RPCで同期更新）
- `ky_record_inventory_move` SECURITY DEFINER RPC: move挿入＋stock_qty再計算
- RLS: オーナーのみ（両テーブル）

### ドメイン層（§50-3 純関数）
- `src/domain/inventory/stockBalance.ts`: sumStockBalance + isAlertTriggered

### アプリ側型
- `types/index.ts`: InventoryMoveKind / InventoryItem / InventoryMove

### Web管理画面
- **lib/types.ts**: KyInventoryMoveKind / KyInventoryItem / KyInventoryMove
- **adminApi.ts**: fetchInventoryItems / upsertInventoryItem / deleteInventoryItem / fetchInventoryMoves / recordInventoryMove
- **AdminInventory.tsx** 新設:
  - 在庫アラートセクション（しきい値割れ品目を赤枠で強調＋発注候補CSVダウンロード）
  - 品目一覧テーブル（現在庫・単位・アラートしきい値・操作ボタン）
  - 品目追加/編集フォーム
  - 入出庫記録フォーム（仕入/廃棄・出庫/棚卸調整・メモ）
  - 品目別入出庫履歴テーブル（日時・種別バッジ・数量±色分け・メモ）
  - 在庫一覧CSVダウンロード
- **AdminLayout.tsx**: ナビに「在庫管理」追加（経費・収支の下）
- **AdminApp.tsx**: /admin/inventory ルート追加

### 未実装（後続Rev・依存あり）
- closeOrder()フック連動（会計確定時にmenu_item_id紐付き品目を自動sale減算）— §32-2集約点の完成待ち
- アプリ側在庫画面（メニュー管理内セクション）
- メニュー編集モーダルの「在庫連動」トグル
- alert_threshold割れ時のプッシュ通知

### 検証
- アプリ `npx tsc --noEmit` EXIT:0
- Web `npx tsc -b` EXIT:0
- **Migration 0040は本番未適用**（承認ゲート）

---

## Rev98（2026-07-11）§49-2 日報（営業日クローズ・DB基盤＋管理Web UI）

### DB（Migration 0041）
- `ky_daily_reports` テーブル新設（business_date UNIQUE/total_revenue/order_count/guest_count/cast_summary jsonb/cash_expected/cash_actual/cash_diff GENERATED/memo/closed_at/closed_by）
- RLS: オーナーのみ
- cash_diff は `cash_actual - cash_expected` の GENERATED ALWAYS AS STORED（自動計算）

### アプリ側型
- `types/index.ts`: DailyReport 型新設

### Web管理画面
- **lib/types.ts**: KyDailyReport インターフェース新設
- **adminApi.ts**: fetchDailyReports（月別）/ upsertDailyReport / deleteDailyReport
- **AdminDailyReports.tsx** 新設:
  - 月次サマリカード（月間売上・営業日数・組数合計・客数合計）
  - 日報一覧テーブル（営業日/売上/組数/客数/現金予定/実査/過不足/メモ/操作）
  - 過不足≠0は赤太字表示
  - 日報作成フォーム（営業日/売上/組数/客数/現金予定/現金実査額/メモ）
  - 締め直し＝既存日報を再編集して closed_at 更新
  - 日報CSVダウンロード
- **AdminLayout.tsx**: ナビに「日報」追加
- **AdminApp.tsx**: /admin/daily-reports ルート追加

### 未実装（後続Rev・依存あり）
- レジ画面からの「営業日を締める」フロー（open伝票チェック→自動集計→確定）
- 締め後の伝票・ky_sales編集ロック
- 日報PDFエクスポート
- 未締めリマインダープッシュ通知
...
- アプリ `npx tsc --noEmit` EXIT:0
- Web `npx tsc -b` EXIT:0
- **Migration 0041は本番未適用**（承認ゲート）

---

## Rev99（2026-07-11）§49-1 卓タイマー（DB基盤＋型＋ドメイン純関数）

### DB（Migration 0042）
- `ky_orders.set_deadline_at timestamptz null` 追加（セット終了予定時刻）
- `ky_tenants.timer_enabled boolean default true` 追加（タイマーON/OFF）
- `ky_tenants.timer_alert_minutes int default 5` 追加（アラート閾値分数）

### ドメイン層（§50-3 純関数）
- `src/domain/order/setTimer.ts`:
  - `calcRemainingSeconds(deadlineIso, nowMs)` — 残り秒数
  - `timerStatus(remainingSeconds, alertMinutes)` — green/yellow/red の色遷移
  - `formatTimer(remainingSeconds)` — `mm:ss` 表示（超過時は `+mm:ss`）

### アプリ側
- `types/index.ts`: Order に `setDeadlineAt` 追加、Tenant に `timerEnabled` / `timerAlertMinutes` 追加
- `services/orders.ts`: OrderRow/rowToOrder に set_deadline_at 追加
- `context/TenantContext.tsx`: timerEnabled / timerAlertMinutes マッピング追加

### Web側
- `lib/types.ts`: KyOrder に `set_deadline_at` 追加、KyTenant に `timer_enabled` / `timer_alert_minutes` 追加

### 未実装（後続Rev）
- レジ画面のタイマーバッジUI（カウントダウン表示・色遷移・延長/会計ポップアップ）
- 伝票オープン時の set_deadline_at 自動初期化
- セット延長明細追加時の set_deadline_at 更新
- 残り5分のローカル通知（expo-notifications）
- タイマーON/OFF設定UI（AdminSettings）

### 検証
- アプリ `npx tsc --noEmit` EXIT:0
- Web `npx tsc -b` EXIT:0
- **Migration 0042は本番未適用**（承認ゲート）

---

## Rev100（2026-07-11）§49-5 指名種別（本指名/場内指名の区別・設定制・既定OFF）

### DB（Migration 0043）
- `ky_tenants.nomination_kinds_enabled boolean default false` 追加
- `ky_menu_items.nomination_kind text null` 追加（'honshimei' | 'jounai' | null）

### ドメイン層（§50-3 純関数）
- `src/domain/nomination/kinds.ts`:
  - `NominationKind` 型（'honshimei' | 'jounai'）
  - `NOMINATION_KINDS` 定数配列（key+label）
  - `nominationKindLabel(kind)` — 種別表示名変換

### アプリ側
- `types/index.ts`: Tenant に `nominationKindsEnabled`、MenuItem に `nominationKind` 追加
- `context/TenantContext.tsx`: nomination_kinds_enabled マッピング追加
- `services/menuItems.ts`: MenuItemRow/rowToMenuItem/MenuItemInput/createMenuItem/updateMenuItem に nomination_kind 追加
- `screens/RegisterScreen.tsx`: handleMenuSave で nominationKind を渡すように修正

### Web側
- `lib/types.ts`: KyTenant に `nomination_kinds_enabled`、KyMenuItem に `nomination_kind` 追加
- `admin/adminApi.ts`:
  - fetchOwnTenant の select に `timer_enabled, timer_alert_minutes, nomination_kinds_enabled` 追加（既存漏れ修正含む）
  - updateTenantFlags の型に `nomination_kinds_enabled` 追加
  - upsertMenuItem の input に `nominationKind` 追加
- `admin/AdminSettings.tsx`: オプション機能セクションに「指名種別（本指名/場内指名）」トグル追加
- `admin/AdminMenu.tsx`:
  - nomination_kinds_enabled ON かつ category='nomination' 時に指名種別ドロップダウン表示（本指名/場内指名/種別なし）
  - メニュー一覧テーブルの品名列に種別バッジ表示

### 設計方針（SPEC §49-5 準拠）
- **第1弾メニュー分割方式**：本指名・場内指名を別メニュー項目（category='nomination'）として登録→§39の3階層バックが種別ごとに独立して効く
- **既定OFF**：コンカフェの多数派（推し単一文化）にはUIを増やさない。キャバ寄り業態の店だけONにする設定制
- nomination_kind をメニューマスタに持たせることでレポート/CSV集計での種別分離の基盤を確保

### その他修正
- `.claude/launch.json`: ポート 5173→5175（vite.config.ts と一致させる修正）

### 検証
- アプリ `npx tsc --noEmit` EXIT:0
- Web `npx tsc -b` EXIT:0
- **Migration 0043は本番未適用**（承認ゲート）

## Rev101（2026-07-11）§47 closeOrderフック＋fetchOwnTenant堅牢化

### ドメイン層（§50-3 純関数）
- `src/domain/inventory/stockBalance.ts`:
  - `buildSaleMoves(orderItems, links)` 追加 — 伝票明細と在庫紐付けから sale 移動行を組み立てる純関数
  - 入力: OrderItemForSale[]（menuItemId+qty）＋InventoryLink[]（itemId⇄menuItemId）
  - 出力: SaleMove[]（itemId, qty=負数）。同一品目は合算
  - 境界値: menuItemId=null→スキップ / 紐付けなし→スキップ / 同一メニュー複数明細→合算

### アプリ側
- `services/orders.ts`:
  - `autoDeductInventory()` 追加 — closeOrder完了後に呼ばれるフック
  - 伝票明細の menuItemId → ky_inventory_items.menu_item_id で在庫品目を検索
  - buildSaleMoves で sale 移動行を計算 → ky_record_inventory_move RPC で登録（stock_qty 自動同期）
  - 在庫0でも会計は止めない（マイナス在庫許容＝§47-2方針）
  - 在庫紐付けが無い明細は自動スキップ（紐付けは任意）

### Web側
- `admin/adminApi.ts`:
  - fetchOwnTenant の select を `*` に変更＋未適用migration列（timer_enabled/timer_alert_minutes/nomination_kinds_enabled）にデフォルト値フォールバック追加
  - migration 0035-0043未適用状態でも管理Webが正常動作するようになった

### 検証
- アプリ `npx tsc --noEmit` EXIT:0
- Web `npx tsc -b` EXIT:0
- 管理WebをBrowserペインで検証：ログイン→設定画面（指名種別トグル確認）→メニュー管理（一覧表示確認）OK

---

## Rev102（2026-07-11）§49-1 卓タイマーUI（レジ画面のタイマーバッジ＋延長/会計ポップアップ）

SPEC §49-1 のUI実装。DB基盤（set_deadline_at列・ドメイン純関数）はRev99で完了済み。

### 変更内容

#### アプリ側
- `screens/RegisterScreen.tsx`:
  - `domain/order/setTimer` の純関数3つ（calcRemainingSeconds/timerStatus/formatTimer）をインポート
  - 1秒ごとのカウントダウン更新（`setInterval` / `nowMs` state）— deadline付きopen伝票がある場合のみ有効化
  - レーンビューの各open伝票カードにタイマーバッジ追加（`mm:ss`カウントダウン表示）
  - 色遷移: 緑（残り十分）→ 黄（残りalertMinutes以内）→ 赤（超過・`+mm:ss`表示）
  - バッジタップで「セット延長（明細追加へ遷移）」「お会計へ」の2択Alert
  - `timerEnabled=false` の店ではバッジ非表示（従来表示のまま）
  - スタイル: `timerBadge`/`timerText` 追加（tabular-nums で数字幅固定）

#### i18n
- `strings.json`: timer.overdue / timer.extend / timer.extendOrCheckout / timer.goCheckout 追加

### 境界値の入出力例（D-5規約・setTimer純関数）
| deadline | now | alertMin | → remainSec | → status | → display |
|---|---|---|---|---|---|
| 2026-07-11T21:30:00Z | 21:20:00 (600s前) | 5 | 600 | green | `10:00` |
| 2026-07-11T21:30:00Z | 21:25:00 (300s前) | 5 | 300 | yellow | `5:00` |
| 2026-07-11T21:30:00Z | 21:32:30 (150s後) | 5 | -150 | red | `+2:30` |
...
## Rev103（2026-07-11）§49-2 営業日クローズUI（レジ画面「営業日を締める」フロー）

SPEC §49-2 のUI実装。DB基盤（ky_daily_reports + 管理Web）はRev98で完了済み。

### 新規ファイル
- `src/domain/accounting/buildDailyReport.ts`:
  - ドメイン純関数 `buildDailyReport(closedOrders)` — 当日のclosed伝票から売上合計/組数/客数/現金売上予定額/キャスト別集計を生成
  - ClosedOrderInput型: subtotal/paymentMethod/castIds
  - I/O依存なし（§50-3 D-1準拠）

- `src/services/dailyReports.ts`:
  - `fetchDailyReport(tenantId, bizDate)` — 既存日報を取得（重複締め防止）
  - `generateReportSummary(tenantId, bizDate)` — closed伝票＋明細からbuildDailyReportで集計
  - `upsertDailyReport(tenantId, bizDate, summary, cashActual, memo, userId)` — 日報行をupsert（closed_at/closed_by付き）

- `src/components/CloseDayModal.tsx`:
  - FormModalShell使用（MODAL-SAFE）
  - フロー: ①open伝票残ありなら警告（先に会計してください） → ②自動集計表示（売上/組数/客数/現金予定額） → ③レジ現金実査額入力→過不足自動表示（0以外は赤） → ④メモ入力 → ⑤「締める」で確定（Alert二重確認）
  - 既に締め済みならチェックマーク表示
  - 会計済み伝票0件なら「伝票がありません」表示

### 変更ファイル
- `screens/RegisterScreen.tsx`:
  - レーンビューのヘッダーに「営業日を締める」ボタン（calendar-lockアイコン）追加
  - CloseDayModal連携（showCloseDay state）

### i18n
- `strings.json`: closeDay.* 16キー追加

### 境界値の入出力例（D-5規約・buildDailyReport純関数）
| 入力 | → totalRevenue | orderCount | cashExpected | castSummary |
|---|---|---|---|---|
| [] (伝票0件) | 0 | 0 | 0 | [] |
| [{subtotal:5000,pm:'cash',casts:['c1']}] | 5000 | 1 | 5000 | [{c1,1,5000}] |
| [{sub:3000,pm:'card',casts:['c1']},{sub:5000,pm:'cash',casts:['c1','c2']}] | 8000 | 2 | 5000 | [{c1,2,8000},{c2,1,5000}] |

### 検証
- アプリ `npx tsc --noEmit` EXIT:0
- Web `npx tsc -b` EXIT:0

## Rev104（2026-07-12）§40(a) シフト表イベント日強調表示

SPEC §40-1 のシフト表画像へのイベント日強調表示。既存の ky_events テーブルをそのまま流用し、新規migrationなし。

### 新規ファイル
- `src/services/events.ts`:
  - `fetchEventsByMonth(tenantId, yearMonth)` — 月のイベント一覧を取得

### 変更ファイル（型・定義）
- `src/shiftTemplates/definitions.ts` / `web/src/shiftTemplates/definitions.ts`:
  - `ShiftPalette` に `eventAccent?: string` 追加（イベント日の強調色。未指定時は `accent` を流用）
- `src/shiftTemplates/shiftData.ts` / `web/src/shiftTemplates/shiftData.ts`:
  - `ShiftEventDay` 型追加 `{ date: string; label: string }`

### 変更ファイル（レンダラー）
- `src/shiftTemplates/ShiftTableRenderer.tsx`（アプリ側）:
  - Props に `eventDays?: ShiftEventDay[]` 追加
  - MonthGrid: イベント日セルに3px太枠＋ラベル帯描画（1名分を削ってラベルスペース確保）
  - WeekRows: イベント日の行頭にカラーラベルバナー描画
- `web/src/shiftTemplates/ShiftTableRenderer.tsx`（Web側）:
  - Props に `eventDays?: ShiftEventDay[]` 追加
  - MonthGrid: 同上
  - WeekRows: 同上
  - DailyLineup: イベント日に上部バナー描画（availH調整込み）

### 変更ファイル（画面・データ供給）
- `src/screens/ShiftImageScreen.tsx`:
  - イベント取得を並列fetch化（`fetchEventsByMonth`）
  - `eventDays` state追加 → ShiftTableRendererに渡す
- `web/src/admin/AdminShiftImage.tsx`:
  - イベント取得をload関数のPromise.allに追加（`fetchEvents`）
  - `eventDays` state追加 → ShiftTableRendererに渡す
...
## Rev107 — §40(c) SNS投稿テンプレート編集UIモーダル（2026-07-12）

### 概要
Rev106のテンプレートエンジンに対する編集UI（SnsTemplateEditor）を実装。
管理Webのシフト表画面「テンプレ編集」ボタンからモーダルが開き、デイリー/マンスリーの投稿テンプレートをカスタマイズ可能。

### 機能
- デイリー/マンスリー切替タブ
- プレースホルダチップ（タップでフッターに挿入: `{{store_name}}`, `{{date}}`, `{{time}}`, `{{name}}`, `{{account}}`, `{{reservation_url}}`, `{{month}}`）
- テンプレートフィールド編集（ヘッダー/時間帯見出し/キャスト1行/フッター）
- 実データ差し込みのリアルタイムプレビュー
- X文字数カウント（280字超で⚠表示）
- 「既定に戻す」ボタンでデフォルトテンプレートに復元
- Supabase `ky_tenants.sns_post_templates` へ保存（updateSnsPostTemplates API）

### 変更ファイル
- `web/src/admin/AdminShiftImage.tsx`: SnsTemplateEditorコンポーネント追加（ファイル末尾）＋onSave型キャスト修正
...
## Rev108 — §50-3 domain純関数切り出し（残り4件）（2026-07-12）

### 概要
§50-3（DDD先行手当て）の切り出し対象表で未実装だった4つのdomain純関数を新規作成。
services層 `materializeRecurringExpenses` を domain関数 `computeMaterializations` に委譲するようリファクタ。

### 新規ファイル
- `src/domain/expense/computeMaterializations.ts`: §42の未実体化月の生成行計算（冪等・過去月不変）
- `src/domain/point/earnRedeem.ts`: §41 calcEarnPoints（切り捨て付与）＋canRedeem（残高ガード）
- `src/domain/payroll/resolveBackEach.ts`: §39 3階層優先度バック解決（RPC SQL同一仕様の写し）
- `src/domain/accounting/calcMonthlyBalance.ts`: §27 月次収支＋年次サマリ

### リファクタ
- `src/services/recurringExpenses.ts`: ローカルの`enumMonths`/`pad2`を削除、`computeMaterializations`に委譲

### 境界値の入出力例（§50-3 D-5）
- **computeMaterializations**: 開始月=2026-01、upTo=2026-03 → 3行生成 / 2月skip済み → 2行 / 既に全月生成済み → 0行
- **calcEarnPoints**: 499円/500ypp → 0pt / 500円 → 1pt / 1499円 → 2pt（切り捨て）
- **resolveBackEach**: backAmount=200 → 200 / backRate=10%,price=1500 → 150 / nomination+defaultRate → null
...
## Rev111 — §32 キャスト成績ビュー 推移グラフ追加（2026-07-12）

### 概要
§26棚卸し#41。既存のキャスト成績ランキング表に加え、過去6ヶ月の推移バーチャート＋キャスト別推移テーブルを追加。
選択中の指標（指名数/ドリンク数/売上貢献/支給総額/出勤日数）に連動して推移を可視化。

### 新規API
- `web/src/admin/adminApi.ts` — `fetchPayrollByRange(tenantId, startYm, endYm)`: 複数月のky_cast_payrollを一括取得

### 変更ファイル
- `web/src/admin/AdminCastPerformance.tsx`:
  - `metricFromRow` / `fmtMetric` ヘルパー関数追加（行単位の指標抽出・書式化）
  - `trendData` state: 過去6ヶ月分のpayrollデータ（load時に並列fetch）
  - `trendMonths` memo: 月別の店舗合計値（選択指標に連動）
  - `castTrend` memo: キャスト別×6ヶ月の推移（上位5名）
  - SVGバーチャート: 6本の棒グラフ（当月=不透明・過去=半透明、年境界でラベル表示）
  - キャスト別推移テーブル: 上位5名の月次推移を表形式で表示
  - 条件分岐を改善: 当月データが空でも推移チャートは表示
...
## Rev112 — キャスト成績 CSVダウンロード追加（2026-07-12）

### 概要
他の分析画面（給与・売上・勤怠・在庫・日報・経費）と同様に、キャスト成績ビューにもCSVダウンロードボタンを追加。
SEC-11（式インジェクション無害化）対応済みのcsv.tsユーティリティを使用。

### 変更ファイル
- `web/src/admin/AdminCastPerformance.tsx`:
  - `downloadCsv` をcsv.tsからimport
  - `handleCsv`: 当月のキャスト別成績（名前/出勤日数/指名数/ドリンク数/売上貢献/支給総額）をCSV出力
  - 日付ナビ内にCSVダウンロードボタンを配置

### 検証
- Web `npx tsc -b` EXIT:0

## Rev113 — §34(c) ご注文予定（事前オーダー＋概算表示）（2026-07-12）

### 概要
客Web予約フォームに「ご注文予定」機能を追加。予約時にメニューを事前選択でき、席料＋注文合計の概算を表示する。
メニューは予約時点の名前・価格をスナップショットとしてjsonbに保存（価格変更耐性＝SPEC snapshot原則）。
「当日にメニューを決める」チェックボックスで事前選択をスキップ可能（デフォルトON＝当日決定）。

### 変更ファイル
- `supabase/migrations/0045_ky_preorder.sql`（新規）:
  - `ky_reservations.preorder` jsonb列追加
  - `ky_reservations.menu_undecided` boolean列追加（DEFAULT false）
  - `ky_menu_items` にanon SELECT RLSポリシー追加（客ページからの読み取り用）
  - `ky_make_reservation` RPCをv2化（12引数: p_preorder/p_menu_undecided追加、既存10引数は後方互換）
- `web/src/lib/types.ts`:
  - `KyPreorderItem` interface追加
  - `KyReservationFull` に `preorder`/`menu_undecided` フィールド追加
- `src/types/index.ts`:
  - `PreorderItem` type追加
  - `Reservation` に `preorder`/`menuUndecided` フィールド追加
- `src/services/reservations.ts`:
  - `ReservationRow` に `preorder`/`menu_undecided` 追加
  - `rowToReservation` マッパー拡張
- `web/src/components/ReservationModal.tsx`（大幅書き換え）:
  - メニュー一覧取得（anon Supabase読み取り）
  - カテゴリ別グループ表示（nomination/cast_drink/drink/food/cheki/other）
  - 数量±ステッパー、needs_cast品目にキャスト選択ドロップダウン
  - 概算カード（席料×セット数＋注文合計＋小計＋免責注記）
  - RPC呼び出しに p_preorder/p_menu_undecided を送信
  - 完了画面に「ご注文予定はスタッフに共有されます」表示
- `web/src/admin/AdminReservations.tsx`:
  - 「注文予定」列追加（品名×数量の一覧 / 当日決定バッジ / —）

### 検証
- App `npx tsc --noEmit` EXIT:0
- Web `npx tsc -b` EXIT:0

## Rev114 — チェックイン→伝票自動作成＋preorderプリフィル（2026-07-12）

### 概要
管理Webで「来店」ボタンを押すと、ステータスをchecked_inに変更するだけでなく、
伝票（ky_orders）を自動作成し、予約時のpreorderスナップショットからオーダー明細を
初期投入するようにした。RegisterScreenのレーンに即座に表示され、スタッフは会計前に
追加・変更が可能。また fetchAllReservations の SELECT に preorder/menu_undecided/
seat_type_id 列が欠落していたのを修正（Rev113の「注文予定」列が正しく表示されるように）。

### 変更ファイル
- `web/src/admin/adminApi.ts`:
  - `fetchAllReservations`: SELECT に `seat_type_id, preorder, menu_undecided` を追加
  - `checkinReservation` 関数新設: ステータス更新→伝票作成(reservation_id紐付き)→preorder明細一括INSERT
- `web/src/admin/AdminReservations.tsx`:
  - `checkinReservation` をimport
  - `handleCheckin` ハンドラ追加（来店ボタンのonClickを差し替え）

### 検証
- Web `npx tsc -b` EXIT:0
- App `npx tsc --noEmit` EXIT:0

## Rev115 — 来店取消時の伝票void処理（2026-07-12）

### 概要
「予約中に戻す」操作時に、Rev114で自動作成された紐付きopen伝票をvoidにする。
これにより来店→取消→再来店で伝票が重複する問題を防止。closed伝票（会計済み）は
voidせず手動対応とする。操作前に確認ダイアログを表示。

### 変更ファイル
- `web/src/admin/adminApi.ts`:
  - `revertCheckin` 関数新設: ステータスをreservedに戻し、reservation_idが一致するopen伝票をvoidに更新
- `web/src/admin/AdminReservations.tsx`:
  - `revertCheckin` をimport
  - `handleRevertCheckin` ハンドラ追加（確認ダイアログ付き）
  - 「予約中に戻す」ボタンのonClickを差し替え
...
## Rev119 — AUD-5是正＝回数券使用のatomic decrement化（2026-07-12）

### 概要
§51監査 🟡 AUD-5 の残り＝回数券（ky_vouchers）を是正し、AUD-5を完了。
- 旧実装はアプリ（src/services/vouchers.ts）とWeb（adminApi.ts）の両 useVoucher がクライアントで読んだ remaining_count に -1 して書き戻す read-modify-write＝2端末同時操作で片方の使用記録が消える（ロスト更新）。
- `ky_use_voucher(p_voucher_id)` RPC（0048）へ移設: `remaining_count = remaining_count - 1` の1文UPDATE＋`remaining_count > 0`／`is_active`／owner検証をWHEREに内包＝競合しても残数が負にならず更新も消えない。負け側は not_usable エラー→既存のcatch＋再読込で実状態に収束。
- ※顧客スタンプ側のatomic化はRev118（0047の ky_close_order v3）で前倒し済み。有効期限チェックは従来どおりUI側の責務のまま（本Revは原子性のみ＝挙動不変）。

### 変更ファイル
- `supabase/migrations/0048_ky_use_voucher.sql`（新規）: ky_use_voucher RPC（SECURITY DEFINER・search_path=public・anon/public REVOKE＋authenticated GRANT）
- `src/services/vouchers.ts` / `web/src/admin/adminApi.ts`: useVoucher をRPC呼び出しへ置換（第2引数 currentRemaining 廃止・WEB13=両面同時変更）
- `src/screens/analytics/VouchersView.tsx` / `web/src/admin/AdminVouchers.tsx`: 呼び出しを1引数へ

### 検証
- アプリ `npx tsc --noEmit` EXIT:0 ／ Web `npx tsc -b` EXIT:0
- **Migration 0048は本番未適用**（承認ゲート・0032〜0048が未適用スタック）。適用時は回数券の使用ボタン一巡＋残数0での連打で not_usable 挙動を実UI確認する

---

## Rev120 — AUD-6/7/8是正＝CSV共通化・招待コード暗号化・アプリ側来店RPC統一（2026-07-12）

### 概要
§51監査の🟡3件＋🟢1件をまとめて是正。
- **AUD-6（SEC-11 CSV式インジェクション）**: AdminExpenses の月次CSV・年次収支CSV が `.join(',')` 手組みだった＝ユーザー定義カテゴリ名にカンマや `=` が含まれると列ズレ/Excel式実行。→ `downloadCsv`（csv.ts の `escapeCell` 経由）へ全面置換。WEB14準拠。
- **AUD-7（非暗号学的乱数）**: 招待コード生成の `Math.random` を `crypto.getRandomValues` へ置換（app/web 両面同時・WEB13）。レート制限（0030 S3パターン）は課金フェーズ前に別Revで追加予定（現時点で authenticated + 7日期限 + 8文字空間 ≒ 5.4×10^11 通り＝ブルートフォースの実害は極めて低い）。
- **AUD-8（app/web来店動作の乖離）**: ReservationsScreen の handleStatusChange を `checked_in` → ky_checkin_reservation RPC / `reserved`（取消）→ ky_revert_checkin RPC に分岐。no_show/cancelled は従来どおり updateStatus。これで管理Webとアプリが同じサーバーロジックを通り、伝票自動作成/void が漏れない形に統一。
- **AUD-12（silently fail catch の理由欠落）**: CastHomeScreen 4箇所＋CastsScreen 1箇所の `// silently fail` に理由コメント＋`if (__DEV__) console.warn(...)` を追加。開発ビルドではエラーがコンソールに出るようになり、本番は従来どおり黙殺（表示補助系のため）。

### 変更ファイル
- `web/src/admin/AdminExpenses.tsx`: import `downloadCsv` 追加。handleCsvExport / handleAnnualCsvExport を2次元配列 + downloadCsv に全面書換え
- `src/services/castInvites.ts` / `web/src/admin/adminApi.ts`: generateCode / generateInviteCode を crypto.getRandomValues に置換
- `src/services/reservations.ts`: checkinReservation / revertCheckin を新設（ky_checkin_reservation / ky_revert_checkin RPC 呼び出し）
- `src/screens/ReservationsScreen.tsx`: handleStatusChange を分岐（checked_in→RPC / reserved→RPC / 他→updateStatus）
- `src/screens/CastHomeScreen.tsx`: 4箇所の `// silently fail` に理由明記＋`if (__DEV__) console.warn(...)` 追加（初期ロード・打刻取得・シフト希望・給与明細）
- `src/screens/CastsScreen.tsx`: 1箇所の `// silently fail` に同上（シフト希望一覧）

### 検証
- アプリ `npx tsc --noEmit` EXIT:0 ／ Web `npx tsc -b` EXIT:0
- AUD-6: `.join(',')` がAdminExpenses内からゼロになったことを確認（全CSV出力は downloadCsv 経由に統一）
- AUD-8: ReservationsScreen の来店動作は0047適用後に実UIで確認（管理Webと同一結果＝伝票が自動作成されることの担保）
- AUD-12: `// silently fail` 全5箇所に理由コメント＋__DEV__警告。意図的黙殺以外の空catch=0をgrep再確認

## Rev121 — §52サブスク・課金 詳細設計をSPECへ新設（2026-07-12・設計のみ＝実装なし）

### 概要
ユーザー指示「サブスク・課金周りの詳細設計を行い計画書に加筆修正して欲しい」。`docs/BILLING_DESIGN.md`（Rev68〜71・639行）をSPECレベルへ上位化し、§42〜§50で増えた全機能のモジュール帰属を確定する **§52** を新設。**矛盾したらSPEC（§52）が勝つ**関係を明文化。

- **§52新設（8節）**:
  - 52-1 確定アーキテクチャ＝チャネル4種（IAP15%/Play15%/Stripe3.6%/振込1.5%）・エンティトルメント単一源泉（ky_billing_subscriptions→recompute RPC=service_roleのみ→plan+entitlements・クライアントは読むだけ）・台帳5表・統一状態機械・Webhook3原則（冪等/API引き直し/5xxリトライ任せ）
  - 52-2 商品モデル＝モジュール選択型×期間。**カタログを8本→9本へ拡張＝customer（お客様）新設**（§45お客様モード＋§41ポイント＋スタンプ）。§34〜§50全機能の帰属表を確定（§46 Q&A AIは無料コア・§47在庫はregister・§43 OCRはexpense回数上限つき・§49系はregister/attendance/sales振り分け・§37は課金対象外）
  - 52-3 トライアル・クーポン＝サーバーサイド一本化（Rev70承認済み）・自動課金なし・クーポン入力は管理Webのみ
  - 52-4 チャネル別要点＝スマホ新法（テキスト誘導0%・QR不設置・リンクアウトはBILL-4判断）・価格差×1.18目安・LP一元化・SAQ A・二重課金防止3層
  - 52-5 UI三面＝アプリPlanCard/ModuleSelector・管理Web#/admin/plan（動的料金プレビュー＋Checkout＋Portal）・有料ゲートの行き止まり禁止
  - 52-6 セキュリティ接続＝SEC-14/FIN-4/AUD-1列GRANT/R13鍵管理/W19試打禁止（検証はテストモード・Sandboxのみ）/§37記帳フック/AUD-7期限
  - 52-7 実装フェーズ＝BILL-0〜4を**§19の58〜61に採番**（前提=専用プロジェクト分離＋§33 Phase B・W21=IAP検証はExpo Go不可）
  - 52-8 ユーザー決定ゲート一覧（BILL-0）＝帰属承認・価格実額・上限数値・SKU・上乗せ率・インボイス・特商法・Stripe開設（本人操作）・規約条項・リンクアウトの10項目＋承認済み1項目
- **§14加筆修正**: 冒頭に§52ポインタ（free/pro 2値表は歴史的叩き台と明記）＋Rev69追補の「⚠️トライアル未承認の仕様分岐」をRev70承認済みへ是正（陳腐化解消）
- **§6 TODO更新**: 課金3項目を§52-8基準へ更新（「ストア標準トライアル」の陳腐化記述も是正）
- **§19採番**: 58=BILL-1課金台帳基盤（クーポン先行稼働含む）／59=BILL-2 Stripe直販／60=BILL-3銀行振込／61=BILL-4 IAP ON化パック

### 変更ファイル
- `SPEC.md`: §52新設・§14ポインタ＋是正・§6更新・§19に58〜61追加
- `REVISION_LOG.md`: 本エントリ

### 検証
- 設計のみ＝コード変更なし（tsc対象外）
- BILLING_DESIGN §13/§18のBILL-0〜4定義と§52-7の整合を突合済み（BILL-1にky-iap-verify/クーポン2表が入る配置も一致）
- 価格実額・帰属の最終承認は全てBILL-0ユーザーゲートに退避＝本Revで価格判断はしていない

## Rev122 — migration 0032〜0048 本番DB適用＋ローカルSQL是正（2026-07-12）

### 概要
Rev117〜121で作成したmigration 17本（0032〜0048）をSupabase本番DB（concafe-yoyaku / ref=rhmuitgbvilqwdevxxox / Role: postgres）にSQL Editorブラウザ自動化で一括適用。適用中に発見した4ファイルのバグをローカル側でも同期修正。

### 適用結果（全17本 Success）

| # | ファイル | 内容 | 適用時修正 |
|---|---|---|---|
| 0032 | ky_make_reservation_cast_validation | §35-2 指名キャスト検証 | なし |
| 0033 | ky_dev_dashboard | §37 開発者ダッシュボード | なし |
| 0034 | ky_shift_submissions | §38 シフト提出 | なし |
| 0035 | ky_menu_back_overhaul | §39 メニュー別バック | なし |
| 0036 | ky_point_settings_rewards | §41(a) ポイント・景品 | `owner_id`→`owner_user_id` ×2箇所 |
| 0037 | ky_recurring_expenses | §42 定期固定経費 | `owner_id`→`owner_user_id` ×2箇所＋`date::timestamp`キャスト |
| 0038 | ky_timecard_punch | §49-3 タイムカード打刻 | なし |
| 0039 | ky_slide_hourly_tiers | §49-4 スライド時給 | なし |
| 0040 | ky_inventory | §47 在庫管理 | `set_updated_at()`→`ky_set_updated_at()` |
| 0041 | ky_daily_reports | §49-2 日報 | `set_updated_at()`→`ky_set_updated_at()` |
| 0042 | ky_orders_set_deadline | §49-1 卓タイマー | なし（0043/0044と一括適用） |
| 0043 | ky_nomination_kinds | §49-5 本指名/場内指名 | なし（一括） |
| 0044 | ky_tenants_sns_post_templates | §40-3 X投稿テンプレ | なし（一括） |
| 0045 | ky_preorder | §34(c) 事前オーダー＋AUD-1/2 | なし |
| 0046 | anon_column_grants | AUD-1 SEC-15 列GRANT横展開 | なし |
| 0047 | ky_checkin_close_rpc | AUD-3/4 チェックイン・会計RPC化 | なし |
| 0048 | ky_use_voucher | AUD-5 回数券atomic decrement | なし |

### 適用中に修正した3種のバグ
1. **`owner_id`→`owner_user_id`**（0036・0037）: RLSポリシーが`ky_tenants.owner_id`を参照していたが、実カラム名は`owner_user_id`（42703エラー）
2. **`date_trunc` IMMUTABLEキャスト**（0037）: `date_trunc('month', date)` がDATE→TIMESTAMPTZ暗黙変換でIMMUTABLE違反（42P17エラー）。`date::timestamp`で明示キャストし`date_trunc(text, timestamp without time zone)`（IMMUTABLE）を使用
3. **`set_updated_at()`→`ky_set_updated_at()`**（0040・0041）: トリガー関数名がプロジェクト内の実名と不一致（42883エラー）

### 変更ファイル
- `supabase/migrations/0036_ky_point_settings_rewards.sql`: owner_id→owner_user_id（2箇所）
- `supabase/migrations/0037_ky_recurring_expenses.sql`: owner_id→owner_user_id（2箇所）＋date::timestampキャスト
- `supabase/migrations/0040_ky_inventory.sql`: set_updated_at→ky_set_updated_at
- `supabase/migrations/0041_ky_daily_reports.sql`: set_updated_at→ky_set_updated_at
- `REVISION_LOG.md`: 本エントリ

### 検証
- SQL Editor実行結果: 全17本「Success. No rows returned」
- anon REST列遮断: `owner_user_id`/`user_id`/`back_rate`→42501 permission denied ✅
- anon REST許可列: `ky_tenants(id,slug,name,genre,is_suspended)`/`ky_menu_items(id,name,price,category)`→HTTP 200 ✅
- authenticated RESTテーブル存在: ky_reservations.preorder/ky_inventory_items/ky_daily_reports/ky_recurring_expenses/ky_shift_requests/ky_point_settings→全HTTP 200 ✅
- RPC関数: PostgRESTスキーマキャッシュ遅延でPGRST202（SQL Editor上では全CREATEが成功しておりキャッシュ自動リロード後に解消）

---

## Rev123 — 遠隔価格フィールド＋お品書きOCR読取り機能（2026-07-12）

### 概要
§53新設。コンカフェ遠隔接客文化に対応するためメニューに遠隔価格欄を追加、お品書き画像からメニュー項目を自動抽出するOCR機能を実装（月20回/テナント制限）。将来のビデオ通話機能は設計メモとして§53-3に記録（MVP外）。

### 変更ファイル

**DB（migration）**
- `supabase/migrations/0049_ky_menu_remote_price.sql`: ky_menu_items.remote_price列追加＋ky_menu_ocr_usageテーブル＋ky_menu_ocr_increment RPC（atomic upsert・月20回上限・security definer・service_role専用）

**Edge Function**
- `supabase/functions/ky-menu-ocr/index.ts`: 新規。認証→テナント解決→使用量チェック→Claude Vision（claude-sonnet-4-20250514）でお品書き画像解析→メニュー項目JSON返却

**アプリ（Expo）**
- `src/types/index.ts`: MenuItem型に`remotePrice: number | null`追加
- `src/services/menuItems.ts`: MenuItemRow/MenuItemInputにremote_price追加・rowToMenuItemマッピング・insert/update反映
- `src/services/menuOcr.ts`: 新規。fetchMenuOcrUsage/canUseMenuOcr/ocrMenuImage
- `src/components/MenuEditModal.tsx`: 遠隔価格入力欄追加（remotePriceStr state・handleSave反映）
- `src/screens/RegisterScreen.tsx`: handleMenuSaveにremotePrice引数追加
- `src/i18n/strings.json`: menu.remotePrice他9キー追加

**管理Web（Vite）**
- `web/src/lib/types.ts`: KyMenuItemにremote_price追加
- `web/src/admin/adminApi.ts`: upsertMenuItemにremotePrice・fetchMenuOcrUsage・ocrMenuImage追加
- `web/src/admin/AdminMenu.tsx`: 遠隔価格入力欄＋テーブル列追加・OCR読取りボタン＋ファイル選択→解析→結果テーブル（チェックボックス選択）→一括追加

**ドキュメント**
- `SPEC.md`: §53新設（遠隔価格設計・OCR設計・将来ビデオ通話メモ）＋§19に㊻追加
- `REVISION_LOG.md`: 本エントリ

### 検証
- `npx tsc --noEmit` EXIT:0（アプリ）
- `npx tsc -b` EXIT:0（管理Web）
- migration文法チェック: RPC引数型・GRANT/REVOKE・RLSポリシー整合確認済み

---

## Rev124 — シフト表拡張3機能の詳細設計をSPECへ追加（2026-07-12）

### 概要
ユーザー指示に基づきシフト表関連の3機能を§22-6/22-7/22-8として詳細設計。設計のみ＝コード変更なし。

### 追加設計（§22拡張）
1. **§22-6 フォント選択20書体**：Google Fonts OFL 5カテゴリ20種（シンプル/ポップ/エレガント/ゴシック/装飾）。ヘッダー＋本文の2系統独立選択。Web=@font-face woff2サブセット・アプリ=expo-font。保存=custom_settings.fonts（jsonb・migration不要）
2. **§22-7 セル透明選択＋プレビューガイド**：cellBg='transparent'で出力時完全透過。プレビューでは薄白15%＋点線ボーダー＋注記バナーで「出力時は透明」を明示。SNS投稿時の黒背景警告ダイアログ付き
3. **§22-8 プレビュー拡大表示**：クリック/タップで全画面ズーム。アプリ=PinchImageViewer流用（urehan正準・Z-1〜Z-15）。Web=CSS transform＋wheel＋drag pan

### §19追加
- ㊿フォント20書体・㊾セル透明・㊿プレビュー拡大（63/64/65番）

### 変更ファイル
- `SPEC.md`: §22-6/22-7/22-8新設＋§19に63〜65番追加
- `REVISION_LOG.md`: 本エントリ
...
## Rev126 — §22-8 シフト表プレビュー拡大表示（Web＋アプリ）（2026-07-12）

### 概要
シフト表作成画面の縮小プレビューをクリック/タップで等倍拡大表示する機能を実装。

### Web側
- **ShiftPreviewOverlay.tsx 新設**：フルスクリーンオーバーレイ（ホイールズーム・マウスドラッグパン・ダブルクリックトグル・Escape/背景クリックで閉じ）
- **AdminShiftImage.tsx 変更**：previewSrc state＋handlePreviewClick（toPngキャプチャ→オーバーレイ表示）・プレビューボックスにcursor:zoom-in/onClick/role追加・案内文を「クリックで拡大表示」に変更

### アプリ側
- **PinchImageViewer.tsx 新設**：urehan（レジさぽっ！）正準からコピー。reanimated 4＋Gesture API・焦点追従ピンチズーム・1指パン＋decay・ダブルタップ3段階（Z-1〜Z-15準拠）
- **ShiftPreviewModal.tsx 新設**：Modal＋SafeAreaView＋閉じるボタン＋PinchImageViewer
- **ShiftImageScreen.tsx 変更**：Pressable wrap＋captureRef→ShiftPreviewModal表示
- **package.json**：react-native-gesture-handler・react-native-reanimated 追加

### 変更ファイル
- `web/src/admin/ShiftPreviewOverlay.tsx` (新規)
- `web/src/admin/AdminShiftImage.tsx` (変更)
- `src/components/PinchImageViewer.tsx` (新規)
- `src/components/ShiftPreviewModal.tsx` (新規)
- `src/screens/ShiftImageScreen.tsx` (変更)
- `package.json` / `package-lock.json` (依存追加)
- `.claude/launch.json` (deploy用devサーバー設定追加)
- `REVISION_LOG.md`: 本エントリ

### 検証
- tsc --noEmit: アプリ側・Web側ともにEXIT:0
- ブラウザ検証: devサーバー起動→ログイン→シフト表作成→プレビュークリック→ズームオーバーレイ正常表示→Escapeで閉じ→コンソールエラーなし

---

## Rev127 — §22-7 透明セル背景（Web）（2026-07-12）

### 概要
店舗テンプレ背景画像モード（PlacementEditor）で、セル背景を「透明」に設定可能にする機能を実装。

### ShiftTableRenderer
- `cellBgWithAlpha` に `'transparent'` ハンドリング追加
- `resolveTransparentBg` 新設：isPreview時=rgba(255,255,255,0.15)、出力時=rgba(0,0,0,0)
- `transparentBorder` 新設：isPreview時のみ点線ボーダー（dashed）表示
- Props に `isPreview?: boolean` 追加（§22-7/22-8共用）
- `CustomPlacement` に透明セル注記バナー（プレビューのみ「出力画像では完全に透明になります」）
- `renderMonthlyCells`/`renderDailyCells` に `cellBgResolved`/`cellBorder` 引数追加

### AdminShiftImage
- プレビュー用 ShiftTableRenderer に `isPreview` 追加
- PlacementEditor: セル背景カラーピッカー横に「透明」チェックボックス追加
- PlacementEditor: transparent時はカラーピッカー無効化＋cellBgAlphaスライダー無効化
- handleDownload: transparent時にSNS黒背景警告ダイアログ（「透明のまま」or「白背景で保存」の2択）

### 変更ファイル
- `web/src/shiftTemplates/ShiftTableRenderer.tsx` (変更)
- `web/src/admin/AdminShiftImage.tsx` (変更)
- `REVISION_LOG.md`: 本エントリ

### 検証
- tsc --noEmit: Web側EXIT:0
- ブラウザ検証: シフト表作成ページ正常表示（既定テンプレモードでisPreview prop影響なし）・コンソールエラーなし

## Rev128 — §22-6 フォント選択20書体（Web＋アプリ型同期）（2026-07-12）

### 概要
シフト表のヘッダー・本文フォントを20書体（Google Fonts）から選択できる機能をWeb管理画面に実装。

### definitions.ts（Web＋アプリ同期）
- `ShiftFontKey` を3キー→23キーに拡張（レガシー3＋新規20）
- `ShiftFontCategory` / `ShiftFontEntry` 型追加
- `FONT_CATEGORY_LABELS`: 5カテゴリの日本語ラベル（シンプル/ポップ/エレガント/ゴシック/装飾）
- `FONT_CATALOG`: 20書体のキー・表示名・CSSファミリー・カテゴリ一覧
- `normalizeFontKey`: レガシーキー→新キー変換（sans-jp→noto-sans-jp等）

### index.html
- Google Fonts CDN `<link>` 追加（20書体・preconnect付き・display=swap）

### ShiftTableRenderer.tsx
- Web側: `FONT_STACKS` を `FONT_CATALOG` から動的展開（レガシー3＋新規20＝23エントリ）
- アプリ側: `FONT_FAMILIES` を `FONT_CATALOG` から動的展開（新書体はシステムフォントへフォールバック）

### AdminShiftImage.tsx
- `ShiftOverrides` に `fontHeader? / fontBody?` 追加
- `parseCustomSettings` にフォントキー復元ロジック追加
- `def` useMemo にフォント上書きマージ追加
- カスタマイズパネルに「ヘッダーフォント」「本文フォント」ドロップダウン追加
  - 5カテゴリ optgroup 分類・各option をそのフォントの font-family で表示
  - テンプレ既定値選択時は override を削除（差分最小化）

### SPEC.md
- §22-6 / §22-7 / §22-8 に完了実績行追加

### 変更ファイル
- `web/index.html` (変更)
- `web/src/shiftTemplates/definitions.ts` (変更)
- `src/shiftTemplates/definitions.ts` (変更・同期)
- `web/src/shiftTemplates/ShiftTableRenderer.tsx` (変更)
- `src/shiftTemplates/ShiftTableRenderer.tsx` (変更・同期)
- `web/src/admin/AdminShiftImage.tsx` (変更)
- `SPEC.md` (変更)
- `REVISION_LOG.md`: 本エントリ

### 検証
- tsc --noEmit: Web側EXIT:0・アプリ側EXIT:0
- ブラウザ検証: フォント変更でプレビュー即時反映確認（Dela Gothic One→ヘッダー太角ゴシック化/Yusei Magic→本文手書き風化）

## Rev129 — §28-3追加修正＝日付チップ折返し防止の強化（2026-07-12）

Rev35で`minWidth:68`に修正済みだったが、`paddingHorizontal:14`（実効40px）では `10/29(水)` 等の長い日付ラベルが依然として折返す場合があった。

### ReservationsScreen.tsx
- `dateChip` スタイル: `paddingHorizontal: 14` → `4`、`minWidth: 68` → `width: 68`（固定幅化でチップ幅の安定化）
- `dateChipText`: `numberOfLines={1}` 追加（万一はみ出す場合も1行に強制）

### 検証
- `npx tsc --noEmit` EXIT:0

## Rev137 — §54 決定反映＋migration 0054本番適用＋Stripe webhook骨組み（2026-07-13）

ユーザー決定事項をSPECに反映＋本番DB更新＋BILL-2先行のEdge Function骨組み。

### SPEC反映
- §49-1 卓タイマー: ✅確定＝店舗設定でON/OFF切替可能
- §49-2 日報: ✅確定＝編集ロック解除にログインパスワード必須（セッション中1回入力で自由編集）・guest_countはレジ集計使用
- §49-4 スライド時給: ✅確定＝給与計算に統合（既定OFF設定制）
- **§54 新設**（第17次）: 複数人会計（均等分割＋同卓伝票分割）／ゲストバック率（インフルエンサー来店時の特別バック）／LP方針（柔軟な特殊計算対応を売りに）
- §19 ロードマップに #66（複数人会計＋ゲストバック率）・#67（Stripe webhook骨組み）追加

### 本番DB
- migration 0054 (`ky_skip_recurring_month` RPC) を本番適用（`db query --linked`）
- migration 履歴テーブル修復（0032〜0054 を全て `applied` としてマーク＝今後の `db push` が重複実行しない）

### 新規ファイル
- `supabase/functions/ky-stripe-webhook/index.ts` — Stripe Webhook受信 Edge Function（verify_jwt=false）。Stripe-Signature HMAC検証＋4イベントのルーティング枠（checkout.session.completed / invoice.paid / customer.subscription.updated / deleted）。ハンドラ本体はstub＝BILL-1/BILL-2 Revで中身実装

### デプロイ
- `ky-stripe-webhook` Edge Function デプロイ済み（--no-verify-jwt）
- STRIPE_WEBHOOK_SECRET 設定後に稼働開始（未設定時は500を返す安全設計）

## Rev138 — §54-1/§54-2 サービス層＋§49-1 タイマー設定UI（2026-07-13）

§54 複数人会計＋ゲストバック率のDB〜サービス層完成＋§49-1タイマーの設定画面。

### migration 0055（未適用・本番適用はユーザー承認待ち）
- `ky_orders.guest_count` (int, NOT NULL DEFAULT 1, CHECK >= 1)
- `ky_orders.is_guest_order` (bool, NOT NULL DEFAULT false)
- `ky_menu_items.guest_back_rate` (numeric(5,2), CHECK 0-100)
- `ky_menu_items.guest_back_amount` (int, CHECK >= 0)
- 排他制約: guest_back_rate と guest_back_amount は同時非NULL禁止

### サービス層
- `menuItems.ts`: MenuItemRow/MenuItemInput/rowToMenuItem/create/update に guestBackRate/guestBackAmount 追加
- `orders.ts`: OrderRow に guest_count/is_guest_order、updateOrderGuestCount/updateOrderGuestFlag 関数
- `dailyReports.ts`: generateReportSummary で guest_count を select して buildDailyReport へ渡す
- `buildDailyReport.ts`: ClosedOrderInput.guestCount 追加、guestCount=伝票ごとの合算（旧=伝票数固定）
- `types/index.ts`: MenuItem/Order 型にフィールド追加済み

### UI
- SettingsScreen: タイマー設定セクション追加（Switch ON/OFF＋分数ステッパー 1〜30）
- TenantContext: updateTenant に timerEnabled/timerAlertMinutes 追加
- i18n: settings.sectionTimer / timerEnabled / timerEnabledSub / timerAlertMinutes

## Rev139 — RegisterScreen 人数ピッカー＋均等分割＋MenuEditModal ゲストバック（2026-07-13）

- RegisterScreen detail view: 人数ステッパー(+/-)＋ゲストフラグトグルボタン（is_guest_order）
- subtotal横に「1人あたり ¥X,XXX」均等分割表示（guest_count > 1の場合のみ）
- MenuEditModal: guestBackRate/guestBackAmount入力欄（排他制御＝片方入力で他方クリア）
- i18n: register.guestCount/guestOrder/perPerson + menu.guestBackRate/guestBackAmount

## Rev140 — §49-2 日報パスワードロック（2026-07-13）

- CloseDayModal: 締め済み日の再編集にログインパスワード入力を要求
- `supabase.auth.signInWithPassword` で現在のユーザーのPWを検証
- 成功で `unlocked=true` → 通常の編集フォーム表示（既存 cashActual/memo をプリフィル）
- セッション中1回入力で自由編集可（モーダル閉→再表示でロック復帰）
- i18n: closeDay.unlockHint/passwordPlaceholder/unlock/wrongPassword

## Rev143 — 管理者→キャスト通知送信UI＋客側Web/管理Webお知らせ（2026-07-13）

### 新規ファイル
- `src/components/SendNotificationModal.tsx` — 管理者→キャスト通知送信フォーム（FormModalShell・タイトル＋本文入力）
- `web/src/components/DevAnnouncementBanner.tsx` — Web版お知らせバナー（audience フィルタ・dismissible）
- `web/src/admin/AdminNotificationBell.tsx` — 管理Web通知ベル（未読ドット＋ドロップダウン一覧・30秒ポーリング・全既読）

### 変更ファイル
- `src/screens/SettingsScreen.tsx` — 通知セクション追加（キャストへ通知を送る）
- `web/src/components/TenantPage.tsx` — 客側Webにお知らせバナー統合（audience='customer'）
- `web/src/admin/AdminLayout.tsx` — ヘッダーに通知ベル＋お知らせバナー統合（audience='admin'）
- `web/src/App.css` — バナー・ベル・ドロップダウンCSS追加
- `src/i18n/strings.json` — notifications.* 10キー追加

---

## Rev142 — 開発者お知らせ＋内部通知システム（2026-07-13）

開発者→全層向けお知らせバナー（audience フィルタ付き）＋キャスト⇄管理者の内部通知（ベルアイコン＋未読バッジ＋一覧パネル）を実装。

### 新規ファイル
- `supabase/migrations/0056_ky_announcements_notifications.sql` — `ky_dev_announcements`（audience text[]・priority・expires_at）＋`ky_notifications`（tenant_id・sender_role/target_role・is_read）
- `src/services/announcements.ts` — fetchAnnouncements（audience contains フィルタ・期限切れ除外）
- `src/services/notifications.ts` — fetchNotifications / fetchUnreadCount / markAsRead / markAllAsRead / createNotification
- `src/components/DevAnnouncementBanner.tsx` — dismissible お知らせバナー（audience prop で admin/cast/customer 切替）
- `src/components/NotificationBell.tsx` — 未読ドット付きベルアイコン（30秒ポーリング）＋パネル開閉
- `src/components/NotificationPanel.tsx` — FormModalShell ベースの通知一覧（既読/全既読/相対時刻）

### 変更ファイル
- `src/screens/ReservationsScreen.tsx` — ヘッダーに NotificationBell(admin) ＋ DevAnnouncementBanner(admin) 追加
- `src/screens/CastHomeScreen.tsx` — タイトル横に NotificationBell(cast) ＋ DevAnnouncementBanner(cast) ＋シフト提出時に admin 向け通知自動作成
- `src/types/index.ts` — DevAnnouncement / InternalNotification / AnnouncementAudience / NotificationSenderRole / NotificationTargetRole 型追加
- `src/i18n/strings.json` — notifications.* 7キー追加

### 本番適用待ち
- migration 0056 は未適用（承認ゲート対象）

---

## Rev141 — §49-4 スライド時給設定UI＋給与計算統合（2026-07-13）

- `SlideTierSettingsModal` 新規作成: ON/OFF切替＋テーブルCRUD＋基準(月間売上/月間指名)選択
- SettingsScreen: スライド時給セクション追加
- `payroll.ts` generatePayrollFromAttendance: `slideEnabled=true` 時に `resolveTierRate` でキャスト別月間指名数から時給を動的解決（フォールバック=baseHourlyRate・売上/指名の高い方を適用）
- i18n: slideTier.* 18キー追加

---

## Rev136 — Web管理画面 白画面バグ修正（ErrorBoundary＋auth race防止）（2026-07-13）

管理Webの画面遷移時ホワイトアウトを修正。原因=①ErrorBoundary欠如（任意のランタイムエラーで全画面消失）②Supabase TOKEN_REFRESHED イベントで session 再設定→テナント再取得→全画面アンマウント。

### 新規ファイル
- `web/src/components/ErrorBoundary.tsx` — React class ErrorBoundary（エラーメッセージ＋再試行/トップ復帰ボタン表示）

### 変更ファイル
- `web/src/App.tsx` — ErrorBoundary で Routes 全体をラップ＋`lazyRetry` wrapper（チャンクロード失敗時にページリロード）
- `web/src/admin/AdminLayout.tsx` — `<Outlet>` を ErrorBoundary でラップ（ページクラッシュがサイドバーを巻き込まない）
- `web/src/admin/AdminApp.tsx` — `TOKEN_REFRESHED` イベント無視（不要な session 更新を防止）＋テナント既取得時は再フェッチスキップ

### 根本原因分析
1. ErrorBoundary 未実装 → 任意の子コンポーネント例外で React ツリー全消失（白画面）
2. `onAuthStateChange` が TOKEN_REFRESHED でも session 更新 → loadTenant useCallback 再生成 → tenantLoading=true → 全管理画面アンマウント（一瞬の白画面＝高速遷移中にクラッシュに発展）
3. lazy() チャンクロードエラー（デプロイ後のキャッシュ不整合）のハンドリングなし

### 検証
- `npx tsc -b` EXIT:0（web）
- dev server で7画面連続遷移テスト: エラーなし・白画面なし

---

## Rev135 — Web管理画面 帳票印刷ボタン追加（給与/売上/勤怠）（2026-07-13）

§44(a)のWeb側対応。AdminExpenses に既にある帳票印刷（window.open→formatted HTML→window.print）と同じパターンを残り3画面に追加。

### 変更ファイル
- `web/src/admin/AdminPayroll.tsx` — 「給与帳票印刷」ボタン追加（キャスト別集計テーブル＋合計支給額）
- `web/src/admin/AdminSales.tsx` — 「売上帳票印刷」ボタン追加（日別売上テーブル＋月間サマリ）
- `web/src/admin/AdminAttendance.tsx` — 「勤怠帳票印刷」ボタン追加（日付×キャスト×状態テーブル）
...
## Rev134 — KA-11是正: ky-menu-ocr JST月キー＋動的media_type（2026-07-13）

KA-11是正。月次使用量カウンターの年月キーをUTC→JST(+9h)に修正し、Claude Vision API呼び出しのmedia_typeを固定'image/png'→リクエストから動的取得に変更。

### 変更ファイル
- `supabase/functions/ky-menu-ocr/index.ts` — `jstYearMonth()`ヘルパー追加・yearMonth計算をJST化・`body.media_type`受け取り(default='image/jpeg')→API呼び出しに反映
- `src/services/menuOcr.ts` — 同様に`jstYearMonth()`追加（サーバーと同じ月キーを使用）・`ocrMenuImage()`に第3引数`mediaType`追加(default='image/jpeg')・呼び出し時に`media_type`をEdge Functionへ送信

### デプロイ
- ky-menu-ocr: 再デプロイ完了（401プローブ検証OK）
- ky-faq-ai: 初回デプロイ完了（Rev131で作成・Rev133で0051本番適用後にアンブロック）
...
## Rev133 — §44(a) expo-print帳票印刷＋KA-10 RPC化＋本番migration 3件適用（2026-07-13）

§44(a) expo-print帳票印刷（OS標準ダイアログ経由・ゼロ設定）を全4分析ビューに実装。KA-10 skipRecurringMonth を BE-4準拠で RPC化。本番Supabaseへ migration 0049/0051/0052 を適用。

### 新規ファイル
- `src/services/print.ts` — expo-print帳票印刷サービス（printPayroll/printExpenses/printSales/printAttendance）。HTML テンプレート生成→`Print.printAsync({html})`でOS印刷ダイアログ起動
- `supabase/migrations/0054_ky_skip_recurring_month_rpc.sql` — KA-10是正。skipRecurringMonth を単一RPC化（skip INSERT + expense DELETE をトランザクション統合・オーナー認証・テナント境界検証）

### 変更ファイル
- `package.json` — `expo-print` 依存追加（`npx expo install expo-print`）
- `src/screens/analytics/PayrollView.tsx` — 印刷ボタン追加（printer-outline アイコン・handlePrint→printPayroll）
- `src/screens/analytics/ExpensesView.tsx` — 同上（printExpenses）
- `src/screens/analytics/SalesView.tsx` — 同上（printSales）
- `src/screens/analytics/AttendanceView.tsx` — 同上（printAttendance）
- `src/i18n/strings.json` — 5キー追加（common.print / payroll.printTitle / sales.printTitle / expense.printTitle / attendance.printTitle）
- `src/services/recurringExpenses.ts` — `skipRecurringMonth()` を2ステップクライアントmutation→`supabase.rpc('ky_skip_recurring_month')`単一呼び出しに変更（BE-4準拠）

### 本番DB適用（ユーザー承認済み）
- migration 0049: `ky_menu_remote_price` 列追加＋`ky_ocr_usage` テーブル＋`ky_increment_ocr_usage` RPC
- migration 0051: `ky_faq_logs`＋`ky_faq_usage` テーブル＋`reserve_ky_faq_slot` RPC
- migration 0052: KA-1/5/7/8/9 セキュリティ是正パック（inventory auth / punch trap / suspended filter / tenant boundary / nomination CHECK）

### 検証
- `npx tsc --noEmit` EXIT:0
- 本番migration 3件: SQL Editor で実行完了確認済み
- migration 0054: ローカルのみ（次回バッチで本番適用）

---

## Rev132 — BILL-0全決定事項の反映＋§35-3 A案確定＋§35-5出勤予定外指名＋i18n確定（2026-07-13）

本セッション（2026-07-13）の全ユーザー決定をSPECに反映。BILL-0ゲートは#6(インボイス)と#10(リンクアウト)を除き全て✅。

### SPEC.md 変更箇所
- §52-8 BILL-0ゲート表: #1帰属承認（8本確定）/#3無料なし（トライアル終了後＝履歴閲覧のみ）/#4簡略9案/#7特商法C方式（レンタルオフィス住所）/#8 Stripe開設済み → 全✅
- §52-2 カタログ: 「（無料コア）」→「base（予約基盤）」＝有料¥12,800。limits廃止（無料プランなし確定で不要化）。「9本」→「8本（将来増可能）」
- §52-2b 価格: OCR月300枚→月100枚（Opus精度優先）・追加300枚→追加50枚
- §35-3: A案（現状維持・オーナー入力は出勤フィルタなし）確定
- §35-5: 出勤予定外キャスト指名希望フロー新設（客→プッシュ→キャスト承諾→shift自動追加）
- §4: i18n確定＝客向け画面のみ5言語(ja/en/zh/ko/fr)・提供者/管理/キャスト=日本語のみ
- §5: 「多言語拡張」→「客向け5言語は✅確定」
- §6: フリー/有料ライン・サブスク価格・i18n範囲・オーダー境界を全て[x]確定済みへ

### メモリ更新（SPEC外）
- `project_kyasuho_competitor_pricing.md`: OCR月300枚→月100枚・追加300→50・AIモデル全確定（shift=Opus/Q&A=Sonnet/receipt OCR=Opus/menu OCR=Sonnet）

### 検証
- SPEC内に矛盾する旧記述が残っていないことをgrep確認

---

## Rev131 — §46 Q&A AI 3層実装＋§45事前設計検証＋監査是正パック（2026-07-13）

§46 SaaS Q&A AIアシスタントを3層アーキテクチャ（FAQ辞書/Sonnet/web_search）で実装し、§45お客様モードの事前設計検証（6チェックポイント全通過）とプロジェクト全体の批判的検証（KA-1〜12）を完了。

### §46 実装（ヘルプチャット両面＋Edge Function＋ナレッジ）
- `docs/faq_knowledge.json` — 157問 Q&A辞書（Layer 1 用・Bigram Dice検索）
- `supabase/migrations/0051_ky_faq_ai.sql` — ky_faq_logs / ky_faq_slots（レート制限）/ reserve_ky_faq_slot RPC
- `supabase/functions/ky-faq-ai/index.ts` — 3層: L1辞書(¥0) / L2 Sonnet+cache / L3 web_search。GUARD_PROMPT＋20回/user/日＋500回/global/日
- `src/components/HelpChatModal.tsx` — アプリ側チャットUI（FormModalShell＋context prop）
- `src/services/faqAi.ts` — クライアントAPI層
- `src/screens/SettingsScreen.tsx` — ヘルプセクション追加（robot-happy-outline アイコン）
- `src/i18n/strings.json` — faq.* キー追加
- `web/src/admin/AdminHelp.tsx` — 管理Webヘルプページ（/admin/help）
- `web/src/admin/AdminApp.tsx` — AdminHelp ルート追加
- `web/src/admin/AdminLayout.tsx` — ナビ「ヘルプ」追加
- `web/src/admin/admin.css` — .admin-help-* スタイル
- `web/src/admin/adminApi.ts` — askFaqAi / FaqChatMessage / FaqAiResult 型追加

### §45 事前設計検証＋SPEC補完
- `SPEC.md` — §45-6（出勤予定外指名）新設 / §45-7（トライアル→ロックUX）追記 / §45-8（customer RLS攻撃面）追記 / §46 第14次書き換え（実装済み3層仕様に） / §19 item 51サブ分割案・item 52完了マーク
- `supabase/migrations/0053_ky_customer_mode.sql` — §45基盤ドラフト（ky_customer_accounts / ky_customer_follows / account_id / ky_point_transactions / mobile_order_token / order_items.status / offschedule列 / 4 RPC）。本番適用は承認ゲート

### 監査是正（KA-1〜12）
- `supabase/migrations/0052_ky_audit_fixes.sql` — KA-1(Critical: ky_record_inventory_move owner照合+revoke) / KA-5(ky_cast_punch record罠修正) / KA-7(anon席種is_suspendedフィルタ) / KA-8(back_each JOINテナント一致) / KA-9(nomination_kind CHECK)
- `docs/CRITICAL_REVIEW_2026-07-12.md` — 全12件所見＋是正状況＋§45検証結果＋§19サブ分割案＋モジュールカタログ矛盾ノート
- `docs/EDGE_FUNCTION_COST_GUARD.md` — 5本横断コスト設計＋Supabase分離計画＋cron代替方針

### 検証
- `npx tsc --noEmit` EXIT:0（app）
- `npx tsc -b` EXIT:0（web）
- Edge Function ky-faq-ai: ローカルのみ（デプロイは承認ゲート）
- migration 0051/0052/0053: ローカルのみ（本番適用は承認ゲート）

---

## Rev130 — G2 i18n準拠＝WEEKDAYS重複解消＋ハードコード日本語のt()化（2026-07-12）

WEEKDAYS定数（6ファイル重複）を`src/utils/dateFormat.ts`へ集約し、ハードコード日本語Alert/UIテキストをi18n t()経由に修正。

### 新規ファイル
- `src/utils/dateFormat.ts` — WEEKDAYS, pad2, formatDateKey, dateLabel, dayLabel を一元管理

### 変更ファイル（WEEKDAYS/dateFormat集約）
- `ReservationsScreen.tsx` — ローカルpad2/formatDate/WEEKDAYS/dateLabel削除→dateFormat import
- `CastsScreen.tsx` — 同上
- `ScheduleScreen.tsx` — 同上
- `CastHomeScreen.tsx` — ローカルWEEKDAY_LABELS削除→WEEKDAYS import
- `analytics/common.ts` — ローカルpad2/WEEKDAYS/dayLabel削除→dateFormat import＋re-export

### 変更ファイル（ハードコード日本語→t()化）
- `VouchersView.tsx` — Alert.alert('保存に失敗しました')→t('common.saveFailed')、'キャンセル'→t('common.cancel')、'削除'→t('common.delete')、'OK'→t('common.ok')、「期限:」「件 有効」→t()パラメータ補間
- `BottleKeepView.tsx` — 同様のAlert修正＋「残量:」「預:」「件 保管中」→t()化
- `ReservationsScreen.tsx` — revertCheckin警告メッセージ→t('reservation.revertClosedWarning')
- `ExpensesView.tsx` — BUILTIN_CATEGORIESのハードコードラベルをTKey参照に変更＋CSV EXPENSE_HEADERをt('csv.expense.headers')経由に変更

### strings.json 追加キー（17件）
- common.saveFailed / voucher.expiryLabel / voucher.activeCount / bottle.remainingLabel / bottle.depositLabel / bottle.activeCount / expense.cat.{purchase,rent,utilities,communication,advertising,costume,supplies,outsourcing,misc} / csv.expense.headers / reservation.revertClosedWarning
...
## Rev144 — §45お客様モード基盤（ロール判定・ホーム画面・i18n 5言語・ナビゲーション）（2026-07-13）

SPEC §45-1〜45-2 の基盤層。migration 0053（DB）は既存＝本Revはアプリ側のロール判定＋画面＋ナビゲーションの実装。

### 新規ファイル
- `src/screens/CustomerHomeScreen.tsx` — フォロー店一覧・slug入力で店追加（FormModalShell）・長押しフォロー解除
- `src/screens/CustomerSettingsScreen.tsx` — 5言語切替・法的情報（利用規約/PP/お問い合わせ）・ログアウト・アカウント削除
- `src/screens/CustomerShopScreen.tsx` — 店舗詳細プレースホルダ（予約/メニュー/ポイントは後続Rev）
- `src/services/customerService.ts` — getFollowedTenants / followTenantBySlug / unfollowTenant

### 変更ファイル
- `src/types/index.ts` — UserRole に `'customer'` 追加
- `src/services/roles.ts` — resolveUserRole: ky_customer_accounts 判定追加（owner→cast→customer→none）＋createCustomerAccount 新設
- `src/screens/RoleSelectScreen.tsx` — 「お客様としてはじめる」ボタン追加
- `App.tsx` — CustomerTabs ナビゲーター（ホーム＋設定2タブ）・DEV FAB の customer はネイティブ画面（WebView廃止）
- `src/i18n/index.ts` — Language型を `'ja' | 'en' | 'zh' | 'ko' | 'fr'` に拡張・CUSTOMER_LANGUAGES エクスポート・StringEntry を ja 必須＋他 Optional に
- `src/context/LanguageContext.tsx` — 5言語保存・復元対応
- `src/i18n/strings.json` — role.customerStart/customerStartDesc ＋ customer.* 24件（5言語）追加

### i18n設計方針（SPEC 2026-07-13確定）
- お客様向け画面のみ5言語（ja/en/zh/ko/fr）＝ customer.* キー
- 提供者アプリ・管理Web・キャストアプリは日本語のみ＝既存キーは ja のみのまま
...
## Rev146 — §45デジタル会員証QR（2026-07-13）

店舗詳細画面ヘッダー右にQRアイコン。タップでモーダルに会員QR（`ky:member:<account_id>`）を大きく表示。店舗スタッフがスキャン→`ky_attach_customer_by_account` RPCで伝票紐付け。

### 変更ファイル
- `App.tsx` — CustomerShopScreen に customerAccountId 伝搬
- `src/screens/CustomerShopScreen.tsx` — QRボタン＋会員QRモーダル追加（react-native-qrcode-svg・白地黒QR）
- `src/i18n/strings.json` — customer.memberQrTitle/memberQrHint/memberQrClose の3キー追加（5言語）
...
## Rev147 — §45アプリ内予約（2026-07-13）

客Webと同じ `ky_make_reservation` RPCを使ったネイティブ予約フロー。店舗詳細画面の日付選択→予約ボタン→時間枠グリッド→フォーム（名前/人数/指名/席種/備考）→RPC送信。アカウント認証済みのためPIN不要。

### 新規ファイル
- `src/lib/timeUtils.ts` — Web版 timeUtils から移植（slotToMinutes/minutesToSlot/countAvailableSeats/getAvailableSlots）
- `src/screens/CustomerReservationModal.tsx` — 予約フォームモーダル（FormModalShell・時間枠選択→詳細入力の2段階UI・指名キャスト/席種選択・席料表示）

### 変更ファイル
- `src/screens/CustomerShopScreen.tsx` — 予約ボタン追加＋shopDetailPlaceholder→shopDetailFutureにリネーム＋予約モーダル接続
- `src/i18n/strings.json` — customer.reserve* 26キー追加（5言語）＋shopDetailFuture追加・shopDetailPlaceholder削除
...
## Rev148 — §45ポイントカード（残高・履歴・特典カタログ）（2026-07-13）

店舗詳細画面にポイントカードセクション追加。ポイント制度ON店舗のみ表示。残高カード＋特典カタログ（交換は店舗スタッフ操作）＋取引履歴（折畳み式）。

### 新規ファイル
- `src/components/CustomerPointSection.tsx` — ポイント残高/特典カタログ/履歴コンポーネント
- `supabase/migrations/0058_ky_customer_point_read.sql` — ky_point_settings/ky_point_rewards の anon SELECT ＋列GRANT（SEC-15）

### 変更ファイル
- `src/screens/CustomerShopScreen.tsx` — CustomerPointSection 組込み
- `src/i18n/strings.json` — customer.point* 13キー追加（5言語）＋shopDetailFutureからポイント言及削除
...
## Rev149 — §45モバイルオーダー（トークン入力→メニュー→カート→注文送信）（2026-07-13）

店舗詳細画面の「モバイルオーダー」ボタンからオーダー画面へ遷移。店舗発行のトークン入力→メニュー閲覧（カテゴリ別・指名/キャストドリンク/ドリンク/フード/チェキ/その他）→数量選択→カートバーで合計確認→ky_submit_mobile_order RPC送信。FIN-9準拠（価格はサーバー側で再解決）。

### 新規ファイル
- `src/screens/CustomerOrderScreen.tsx` — トークン入力＋メニュー閲覧＋カート＋注文送信画面

### 変更ファイル
- `App.tsx` — CustomerOrderScreen import＋orderRouteステート＋ルーティング追加
- `src/screens/CustomerShopScreen.tsx` — onOpenOrder prop追加＋「モバイルオーダー」ボタン＋Props型拡張
- `src/i18n/strings.json` — customer.order* 20キー追加（5言語）
...
## Rev151 — §45批判的レビュー是正（RLS/GRANT/削除カスケード/i18n/PP/デッドコード）（2026-07-13）

Rev149-150の批判的チェックで検出された17件のうち残修正分を一括適用。

### セキュリティ（S-1〜S-5）— migration 0059
- authenticatedロール用のSELECTポリシーを10テーブルに追加（お客様モードはauthenticatedで動作するがポリシーがanonのみだった）
- SEC-15列レベルGRANTをauthenticatedにも適用（ky_unlock_windows, ky_seat_types含む全対象テーブル）
- 全ポリシーにsuspendedテナント除外条件を付加

### ライフサイクル（L-1）— migration 0060
- ky_delete_account v3: §45追加テーブル（ky_point_transactions, ky_faq_logs, ky_faq_usage）の削除カスケード追加

### 計算正確性（C-2）— migration 0061
- ky_customer_point_balance RPC新設（サーバー側SUM。limit(50)の履歴クライアント合算→全件サーバー集計）
- CustomerPointSection: RPCを第一候補として呼び出し、フォールバックでクライアント合算

### コード品質（C-1, C-4, C-5, C-6, C-7）
- CustomerReservationModal: セット数選択UIを非表示（RPCにp_setsパラメータがない＝C-1）
- CustomerOrderScreen/CustomerShopScreen/CustomerPointSection: fetch系のtry/finally統一（C-4）
- CustomerOrderScreen: トークン入力欄にKAV+KeyboardDoneBar追加（C-5）
- CustomerShopScreen: QRボタンをcustomerAccountId存在時のみ表示、fallback値をinvalidに（C-6）
- CustomerShopScreen/CustomerHomeScreen/App.tsx: 未使用slugプロップのチェーン全除去（C-7）

### 国際化（I-1, I-2）
- CustomerShopScreen: WEEKDAYS_JAハードコードを`common.weekday*`i18nキーに置換（I-1）
- strings.json: common.error/cancel/loading/kbdDone、settings.signOut系/terms/privacy/deleteAccount、role.customerStart/Descにen/zh/ko/fr追加（I-2）
- strings.json: common.weekdaySun〜Satを5言語で新設

### 法務（L-2, L-3）
- privacyPolicy.ts §1: (7)Q&Aアシスタント利用情報を追記（L-2）
- privacyPolicy.ts §5: (2)削除対象にQ&Aログ・利用回数を追記、(4)店舗側台帳の保持・紐づけ解除を明記（L-3）

### 変更ファイル
- `supabase/migrations/0059_ky_authenticated_customer_grants.sql` — 新規
- `supabase/migrations/0060_ky_delete_account_v3.sql` — 新規
- `supabase/migrations/0061_ky_customer_point_balance_rpc.sql` — 新規
- `src/components/CustomerPointSection.tsx` — RPC呼出し・try/finally・デッドコード除去
- `src/screens/CustomerReservationModal.tsx` — セット数UI非表示・try/finally
- `src/screens/CustomerOrderScreen.tsx` — try/finally・KAV+KeyboardDoneBar
- `src/screens/CustomerShopScreen.tsx` — QR条件分岐・slug除去・WEEKDAYS_JA→i18n
- `src/screens/CustomerHomeScreen.tsx` — slug除去
- `App.tsx` — slug除去
- `src/i18n/strings.json` — 曜日7キー新設・12キー多言語化
- `src/data/privacyPolicy.ts` — §1(7)・§5(2)(4)追記
...
## Rev150 — §45法務改訂（PP・利用規約にお客様モード追記）（2026-07-13）

お客様モード（§45）追加に伴い、プライバシーポリシー・利用規約をお客様アカウント関連のデータ収集・利用・削除に合わせて改訂。

### 変更ファイル
- `src/data/privacyPolicy.ts` — §1に(6)お客様アカウント情報追加、§2に(5)お客様モード機能追加、§3に(4)会員QR提示時の共有情報追加、§5に削除対象データとしてお客様関連データ追記。lastUpdated→2026-07-13
- `src/data/termsOfUse.ts` — §2にお客様モード（予約・モバイルオーダー・ポイント）追記、§6(1)利用客を主体に追加・(2)利用客削除時のカスケード範囲追記。lastUpdated→2026-07-13
...
## Rev152 — migration修正＋本番適用完了（0053〜0061）（2026-07-13）

Gate C（migration本番適用）完了。

### 修正内容
- **0057**: 0053のALTER TABLE（accepts_offschedule_nomination, nomination_type）が本番で未実行だった問題に対し`IF NOT EXISTS`ガードを追加（補完）
- **0059**: ky_unlock_windowsの列GRANTが旧スキーマ名（day_of_week/start_slot/end_slot/close_minutes_before/capacity）で記述されていたバグを、実テーブル列（date/open_from/close_at/seats/set_minutes）に修正

### 本番適用状況
- 0053: 適用済み（メタデータのみ・DDLは0057で補完）
- 0054〜0058: 前セッションで適用済み
- 0059（authenticated向け公開読取りポリシー＋列GRANT横展開）: ✅適用
- 0060（ky_delete_account_v3＝カスケード削除拡張）: ✅適用
- 0061（ky_customer_point_balance_rpc）: ✅適用

### 変更ファイル
- `supabase/migrations/0057_ky_customer_read_grants.sql` — 0053列補完ガード追加
- `supabase/migrations/0059_ky_authenticated_customer_grants.sql` — ky_unlock_windows列名修正
