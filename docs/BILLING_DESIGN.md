...
- ⚠️ **仕様分岐＝ユーザー確認ゲート**: 2026-07-06 決定「ASC Introductory Offer / Play free trial を採用」を**「サーバーサイド・トライアルに変更（ストア側オファーは設定しない）」へ差し替える提案**。承認されるまで SPEC §14 の該当記述は現状維持。（代替案: ストアオファー維持＋チラシは Apple Offer Codes を印刷する方式もあるが、iOS/Web でコード体系が分裂しチラシに2種類のコードを書くことになるため非推奨）
...
| Phase | 追加内容 |
|---|---|
| BILL-0 | モジュール境界の確定・個数→価格テーブル・半年/年割引率・iOS SKUラインナップ（フル21か簡略9か）・**トライアル方式変更（§17-1 仕様分岐）の承認** |
...
- [ ] トライアルのサーバーサイド一本化（2026-07-06 のストアオファー決定の差し替え・§17-1）
...
| トライアル=ASC Introductory Offer / Stripe trial_period_days | **サーバーサイド・トライアル（channel='promo' 行）へ一本化を提案**（承認待ちの仕様分岐） |
