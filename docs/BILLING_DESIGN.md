...
| `shift` | シフト表スタジオ | シフト表生成・テンプレ40種・AIデザイン・店舗テンプレ取込・SNS投稿（§22・§31） |
...
### 16-3. チャネル別価格差（Apple/Play 上乗せ）と料金案内の場＝LP【2026-07-10 ユーザー指示・Rev71】

- **① 料金の案内は LP 作成時に行う**。料金表・プラン説明・チャネル別の申し込み方法は **LP（アプリ外）を正式な案内場所**とする。
  - アプリ内には価格表を置かない（§17-5 のテキスト誘導規定と整合。「詳しくは『きゃすりん Web』で検索」→ LP で料金を見る動線）。
  - ストア説明文にも金額の固定文字を書かない（ルールR29＝`localizedPrice` のみ）。LP に一元化することで価格改定時の更新箇所も LP 1箇所になる。
- **② Apple（IAP）／Google Play 経由はストア手数料分を上乗せして高く設定する**。
  - Web直販（カード/振込）はStripe手数料≒3.6%、IAPはSmall Businessでも15%。同額にするとIAP経由だけ粗利が毀損するため、**IAP/Play の SKU 価格は Web 基準価格より高く設定**して手数料分を吸収する。
  - 参考計算: 15%を丸ごと吸収するなら Web価格 ÷ 0.85 ≒ **×1.18**（例: Web ¥2,400 → IAP ¥2,800級）。実際の上乗せ率・丸め（ストア価格Tierへの吸着）は**ユーザー決定ゲート（BILL-0）**。
  - R35（地域別価格倍率＝JP基準・米×1.4/韓×1.2）は**この上乗せ後の JP 価格を基準**に掛ける（独立した2段階）。
- **③ 価格差の説明も LP で行う**。「Webからのお申し込みが最もお得です（アプリ内購入はストア手数料の関係で価格が異なります）」の旨を LP に明記する。
  - **アプリ内では価格差に言及しない**（安価なWeb価格への誘導＝steering は §17-5 のリンク規制と同根の審査リスク。アプリ内はIAP価格だけを `localizedPrice` で表示）。LP・チラシ・POP・管理Web はアプリ外なので自由に書ける。
  - 景表法注意: Web価格を「割引価格」と表現しない（二重価格表示の有利誤認リスク）。**各チャネルの正価が異なる**という事実表示に留める（「Web価格 ¥X／App Store価格 ¥Y」の併記型）。
- 実装への影響: 価格テーブル（§15-3）は**チャネル別に2系統**（web / store）持つ。台帳・entitlements はチャネル非依存のまま（金額差は Price/SKU 定義の話で権利内容は同一）。
...
| Phase | 追加内容 |
|---|---|
| BILL-0 | モジュール境界の確定・個数→価格テーブル・半年/年割引率・iOS SKUラインナップ（フル21か簡略9か）・**Apple/Play上乗せ率の実額（§16-3）**。~~トライアル方式変更（§17-1 仕様分岐）の承認~~ → ✅ **2026-07-10 承認済み（Rev70）** |
...
- [ ] Apple/Play 上乗せ率の実額（÷0.85≒×1.18 目安・ストア価格Tierへの丸め＝§16-3）
- [ ] LP の料金表・チャネル別価格差の文言（§16-3 ③の表現・LP1-8 準拠で LP 制作時に確定）
